...
\begin{question}
    ...
    \begin{question}
        ...
        \begin{enumerate}
            \item The bank must be able to physically trade the asset
            \item The risk of the asset must be managed by the bank's trading desk
        \end{enumerate}
    \end{question}
    ...
    \checklist{交易账户资产需同时满足可交易性和交易台管理两个条件}
    ...
    \begin{explanation}
        \textbf{A选项错误。}
        \begin{itemize}
            \item 这个说法是正确的。巴塞尔协议I和巴塞尔协议II.5都使用99\%置信水平的VaR作为风险度量方法。
        \end{itemize}
        
        \textbf{B选项错误。}
        \begin{itemize}
            \item 这个说法是正确的。FRTB使用97.5\%置信水平的预期短缺（Expected Shortfall）作为风险度量方法。
        \end{itemize}
        
        \textbf{C选项正确。}
        \begin{itemize}
            \item 这个说法是错误的。FRTB不需要在预期短缺计算中添加压力VaR。实际上，是巴塞尔协议II.5要求添加压力VaR作为补充风险度量。
        \end{itemize}
        
        \textbf{D选项错误。}
        \begin{itemize}
            \item 这个说法是正确的。巴塞尔协议I中10天期限的市场风险资本要求确实包含了最近一段时间（通常为1-4年）的数据。
        \end{itemize}
    \end{explanation}
    
    \checklist{FRTB不需要压力VaR而巴塞尔II.5需要}
    ...
    \checklist{相同置信水平下ES要求的经济资本高于VaR}
