...
\begin{explanation}
    ...
    \begin{itemize}
        \item 计算过程如下：
        \begin{enumerate}
            \item 压力下的预期损失 = EAD × PD × LGD
            \item EAD = 1,000,000
            \item PD = 4\%
            \item LGD = 50\%
            \item 压力下的预期损失 = 1,000,000 × 4\% × 50\% = 0.02 million
        \end{enumerate}
    \end{itemize}
...
\checklist{压力下的预期损失计算需要考虑风险敞口、违约概率和违约损失率}
...
\checklist{公司规模越大，分析成本越低，因为大型公司有更多公开信息可供参考}
...
\checklist{CreditMetrics模型用于计算由于信用迁移导致的投资组合价值变化}
...
\checklist{名义贷款是一种基于借款人声誉的定性分析技术}
...
\begin{explanation}
    ...
    \begin{itemize}
        ...
        \begin{enumerate}
            ...
            \begin{itemize}
                \item LGD = 风险敞口 - 回收金额
                \item LGD = \$500,000 - \$350,000 = \$150,000
            \end{itemize}
            \item 计算预期损失：
            \begin{itemize}
                \item 违约概率 = 2\%
                \item 预期损失 = 违约概率 × LGD
                \item 预期损失 = 2\% × \$150,000 = \$3,000
            \end{itemize}
        \end{enumerate}
    \end{itemize}
    ...
    \textbf{D选项错误。}
    \begin{itemize}
        \item 这个选项是违约损失（LGD）的金额，而不是预期损失。预期损失需要考虑违约概率。
    \end{itemize}
\end{explanation}

\checklist{预期损失的计算需要考虑风险敞口、回收率和违约概率}
...
\checklist{信用风险评估的四个主要要素：债务人能力、外部环境、信用工具特征和风险缓释工具}
    ...
    \begin{itemize}
        ...
        \begin{enumerate}
            ...
            \begin{itemize}
                ...
                \item 长期负债的一半 = 20 million × 0.5 = 10 million
                \item K = 40 million + 10 million = 50 million
            \end{itemize}
            \item 计算资产价值（A）：
            \begin{itemize}
                \item 股权市值 = 1 million shares × \$10/share = 10 million
                \item 总负债 = 40 million + 20 million = 60 million
                \item A = 10 million + 60 million = 70 million
            \end{itemize}
            \item 计算资产波动率（σA）：
            \begin{itemize}
                \item σA = 20\% × 70 million = 14 million
            \end{itemize}
            \item 计算违约距离（DD）：
            \begin{itemize}
                \item DD = (A - K) / σA
                \item DD = (70 million - 50 million) / 14 million = 1.429
            \end{itemize}
        \end{enumerate}
    \end{itemize}
...
\checklist{KMV模型中的违约距离计算需要考虑债务阈值、资产价值和资产波动率}
...
\checklist{风险转移是指风险和收益从一组市场参与者转移到另一组持有不同公司资本结构头寸的参与者}
...
\checklist{现金流量表对银行信用分析的用处较小，因为银行的经营活动本身就是处理现金和现金等价物}
...
\checklist{定量信息可以用具体数字衡量，而定性信息需要主观评估}
...
\begin{explanation}
    \textbf{A选项错误。}
    \begin{itemize}
        \item 个人（消费者）信用分析确实适合自动化和使用评分模型，因为：
        \begin{enumerate}
            \item 消费者信用风险可以用有限的变量来评估
            \item 可以使用统计工具来关联风险
            \item 可以建立标准化的评分模型
        \end{enumerate}
    \end{itemize}
    ...
    \textbf{B选项错误。}
    \begin{itemize}
        \item 非金融企业的信用分析确实比消费者更详细和"手动"，因为：
        \begin{enumerate}
            \item 需要评估流动性
            \item 需要分析现金流
            \item 需要评估短期盈利能力和长期盈利能力
            \item 需要分析偿付能力和资本状况
        \end{enumerate}
    \end{itemize}
    ...
    \textbf{C选项正确。}
    \begin{itemize}
        \item 金融机构的信用分析与非金融企业有共同点，但有两个关键区别：
        \begin{enumerate}
            \item 资产质量非常重要
            \item 现金流不是关键指标
        \end{enumerate}
    ...
    \begin{itemize}
        \item 主权信用分析确实包括：
        \begin{enumerate}
            \item 国家风险分析（主要是政治动态和经济状况）
            \item 系统性风险分析（包括监管制度和金融体系）
        \end{enumerate}
    \end{itemize}
\end{explanation}

\checklist{金融机构信用分析中资产质量很重要，而现金流不是关键指标}
