...
\begin{question}
    ...
    一位风险经理正在考虑写一个6个月的美式看跌期权，标的股票不支付股息。当前股票价格为USD 50，期权的行权价为USD 52。为了找到无套利价格，经理使用一个两步二叉树模型，其中股票价格可以上涨或下跌20\%每个周期。年无风险利率为12\%，连续复利。股票价格在单步中上涨的风险中性概率是多少？
\end{question}
...
\begin{question}
    ...
    John使用四步二叉树模型对一个四年到期的看涨期权进行估值；即，每个时间步长为一年。无风险利率为5.0\%，标的资产的波动率为30.000\%。使用这些假设，他惊讶地发现模型中上涨的概率为50.0\%；即，\( p = d = 0.50 \)。然而，他忘记包括标的资产将支付连续股息3.0\%每年的假设。这个假设会改变他的模型中下跌（d）运动的概率多少？
\end{question}
...
\begin{question}
    ...
    当前股票价格为\$10，已知三个月后股票价格将为\$13或\$7。无风险利率为4\%每年。三个月（T = 0.25）欧式看涨期权在股票上的无套利价格是多少？（注意：这不包括关于股票波动率的假设）
\end{question}
...
一位交易员持有执行价格为\$50的美式看跌期权。标的资产为股票，当前价格为\$40。使用一个一步二叉树模型评估期权。假设股票价格将在6个月内上涨或下跌\$8，无风险利率为6.2\%，这个美式看跌期权的价值是多少？
\end{question}
...
\\一只股票当前价格为\$32，波动率为15\%，每年支付2.0\%的股息（连续复利）。无风险利率为2.0\%。我们使用一个十二步二叉树模型对一个一年到期的美国看跌期权进行估值；即，每个时间步长为一个月。风险中性概率下跌（1-p）是多少？
\end{question}
...
\begin{question}
    ...
    \\一家科技公司的股票当前价格为\$45，波动率为20\%。公司预计在未来两年内支付连续股息收益率1.5\%每年。我们使用一个两步二叉树模型对一个两年到期的欧式看涨期权进行估值；即，每个时间步长为一年。无风险利率为5.0\%。二叉树模型中最低股价的节点股价是多少？
\end{question}
...
\begin{question}
    ...
    一只不支付股息的股票当前交易价格为\$100，股票的年波动率为25\%，连续复利无风险利率为4\%。存在一个四年到期的欧式看涨期权，执行价格为\$120。假设股票价格每年会按比例上涨或下跌，并且股票上涨的概率约为55\%，这个欧式看涨期权的价值是多少？
\end{question}
...
\begin{question}
    ...
    S\&P 500指数当前为4,500.0，波动率为30.0\%，股息收益率为1.5\%。无风险利率为2.5\%。如果我们使用一个两步二叉树模型，哪一个最接近一个六个月到期的欧式看涨期权，执行价格为4,700.0；即，看涨期权处于正好200的价外状态？
\end{question}
...
\begin{question}
    ...
    \\给定条件：\( S = \text{USD } 120 \)，\( T = 1 \)，\( X = \text{USD } 110 \)，\( r = 4.5\% \)，\( u = 1.25 \)，\( d = 0.75 \)。这是一个欧式看跌期权，行权条件为\( X > S \)。使用连续复利，最优对冲比率和期权的价值是多少？
\end{question}
    
\begin{choices}
    \item Hedge ratio: -0.33; Option value: USD 7.84\\
    对冲比率：-0.33；期权价值：USD 7.84
    \item Hedge ratio: -0.33; Option value: USD 8.00\\
    对冲比率：-0.33；期权价值：USD 8.00
    \item Hedge ratio: -0.40; Option value: USD 7.50\\
    对冲比率：-0.40；期权价值：USD 7.50
    \item Hedge ratio: -0.40; Option value: USD 7.85\\
    对冲比率：-0.40；期权价值：USD 7.85
\end{choices}
...
\begin{question}
    ...
    \\假设有一只股票不支付股息的欧式看涨期权。当前股票价格为USD 100，到期为1年，执行价格为USD 120。如果无风险利率为4.5\%，\( u = 1.25 \)，\( d = 0.75 \)，使用一个一步二叉树模型，最优对冲比率和期权价格是多少？如果期权的市场价格为USD 3，存在什么套利机会？
    \end{question}
    
    \begin{choices}
    \item Hedge ratio: 0.1; Option price: USD 2.82; Arbitrage profit: USD 0.18\\
    对冲比率：0.1；期权价格：USD 2.82；套利利润：USD 0.18
    \item Hedge ratio: 0.2; Option price: USD 3.00; Arbitrage profit: USD 0.00\\
    对冲比率：0.2；期权价格：USD 3.00；套利利润：USD 0.00
    \item Hedge ratio: 0.1; Option price: USD 2.50; Arbitrage profit: USD 0.50\\
    对冲比率：0.1；期权价格：USD 2.50；套利利润：USD 0.50
    \item Hedge ratio: 0.2; Option price: USD 2.82; Arbitrage profit: USD 0.18\\
    对冲比率：0.2；期权价格：USD 2.82；套利利润：USD 0.18
    \end{choices}
...
\begin{question}
    ...
    一位风险经理Mark正在考虑写一个6个月的美式看跌期权，标的股票不支付股息。当前股票价格为USD 50，期权的行权价为USD 52。为了找到无套利价格，Mark使用一个两步二叉树模型，其中股票价格可以上涨或下跌20\%每个周期。Mark认为股票价格有80\%的概率上涨每个周期，有20\%的概率下跌。年无风险利率为12\%，连续复利。期权的无套利价格最接近的值是多少？
    \end{question}
...
\begin{question}
    ...
    一位投资者使用二叉树模型对一个一年期的期权进行估值。原始模型假设每个时间步长为0.5年，但调整后的模型将时间步长改为1/12年。假设其他参数，如资产波动率和无风险利率，保持不变。关于\( u \)，\( d \)和风险中性概率\( P \)的变化，哪一项陈述是正确的？
    \end{question}
...
\begin{question}
    ...
    一位初级交易员在衍生品交易台评估二叉树模型和布莱克-斯科尔斯-默顿（BSM）模型在提高复杂期权定价准确性方面的收敛特性。交易员特别关注二叉树模型中步数对收敛到BSM模型的影响。哪一项陈述正确描述了这种收敛？
    \end{question}
    ...
    二叉树模型的定价结果总是与BSM模型显著不同，增加步数不会影响收敛。
    ...
    随着步数的增加，二叉树模型的定价结果变得越不稳定，导致与BSM模型更大的差异。
    \end{choices}
...
\begin{question}
    ...
    当前股票价格为\$25。一个六个月到期的看跌期权，行权价为\$20。\( N(d_1) = 0.9737 \)和\( N(d_2) = 0.9651 \)。如果标的股票的年标准差为25\%，当前连续复利无风险利率为4.25\%，看跌期权的Black-Scholes-Merton值最接近的值是多少？
\end{question}
...
\begin{question}
    ...
    一个一年到期的欧式看涨期权，欧元兑美元的行权价为\$1.40，当前汇率为EUR/USD \$1.34。美国无风险利率为4\%，欧元区无风险利率为3\%。现货汇率的波动率为30\%每年。看涨期权的价值是多少？
\end{question}
...
\begin{question}
    ...
    一位金融公司的期权交易员正在审查布莱克-斯科尔斯期权定价模型的基本原理，以确保对衍生品的准确定价。交易员意识到该模型基于几个关键假设。以下哪一项是布莱克-斯科尔斯期权定价模型的基本假设之一？
\end{question}

\begin{choices}
    \item The stock price follows a geometric Brownian motion (GBM) which is a continuous process without jumps. \\
    股票价格遵循几何布朗运动（GBM），这是一个连续的过程，没有跳跃。
    ...
    股票的预期真实（风险）回报率是已知的，并且期权的价值是这个回报率的递增函数。
\end{choices}
...
\begin{question}
    ...
    一家不支付股息的科技公司的首席财务官要求一位财务分析师评估一项计划，向其员工授予股票期权。公司有8000万股流通股。根据提议，公司将发行500万股员工股票期权，每份期权赋予持有人以执行价格USD 75购买公司股票的权利。员工股票期权将在5年后到期。使用Black-Scholes-Merton模型，一个五年期看涨期权，行权价格为SGD 5.20，在股票上的价值是多少？哪一项是对一个员工股票期权假设看涨期权正确定价的最佳估计？
\end{question}
...
\begin{question}
    ...
    一位FRM（金融风险管理师）的Jasmine Tang正在使用一个简单的称为“successive bisection”的方法来确定一个交易期权的隐含波动率。Jasmine从零波动率开始（这给出了一个低于期权市场价格的BSM价格），然后是30\%波动率（这给出了一个高于期权市场价格的BSM价格）。然后她取这两个波动率的平均值，即15\%，作为新的波动率，并使用BSM公式计算期权价格。修订后的期权价格现在更接近市场价格，但仍然有点低。哪一项陈述正确描述了Jasmine接下来应该执行的步骤？
\end{question}
...
\begin{choices}
    ...
    用15\%替换30\%，重新计算使用0\%和15\%的平均波动率，使用新的平均值使用BSM公式计算期权价格，并与市场价格进行比较。
    ...
    用15\%替换0\%，重新计算使用30\%和15\%的平均波动率，使用新的平均值使用BSM公式计算期权价格，并与市场价格进行比较。
...
\begin{question}
    ...
    \begin{question}
        ...
        一位量化分析师正在开发一个波动率估计模型，用于一个新的交易策略。分析师的同事建议使用60天的历史数据来估计高股息股票的年波动率。哪一项陈述最准确地解决了这种方法可能存在的问题？
        \end{question}
        ...
        估计期太短，因为标准做法要求至少90-180天的数据来计算可靠的年化波动率。
        ...
        这种方法忽略了股息影响，但使用未经调整的除息日价格数据足以进行波动率估计。
        ...
        估计期和股息处理都是适当的，因为短期数据更好地捕捉当前市场情况。
        ...
        这种方法有两个根本缺陷：相对于预测期，历史数据不足，以及未能解决股息引起的价格变动。
        \end{choices}
        ...
        \begin{question}
            ...
            \\两位量化分析师正在开发一个布莱克-斯科尔斯模型，用于定价股票期权。他们已经识别出几个可能影响模型可靠性的市场观察：
            
                I. 历史数据显示股票波动率随时间显著变化。
                II. 股票收益表现出重尾分布（leptokurtosis）。
                III. 股票支付季度股息，而不是连续股息收益率。
                IV. 正在定价的期权是美式期权，而不是欧式期权。
          
            哪一项组合因素对基本布莱克-斯科尔斯模型实施提出了根本性挑战？
            \end{question}
            ...
            \begin{question}
                ...
                一位衍生品交易员在一家做市商公司评估他们的布莱克-斯科尔斯定价模型，观察到几个市场异常。交易员需要识别哪一项市场特征需要对现有定价框架进行根本性改变。哪一项观察代表对公司基本布莱克-斯科尔斯实施的最重大挑战？
            \end{question}
...
\begin{question}
    ...
    \\    一位衍生品交易商与其客户积极交易各种标的资产的期权。该公司希望应用Black-Scholes-Merton（BSM）模型来为期货合约上的看涨期权定价。相关数据如下：
    
    \begin{itemize}
    \item 当前期货价格：EUR 65
    \item 期权的执行价格：EUR 70
    \item 期权的到期时间：9个月
    \item 标的期货合约的到期时间：24个月
    \item 连续复利年化无风险利率：4\%
    \item \( N(d1) \): 0.4821
    \item \( N(d2) \): 0.3625
    \end{itemize}
    
    使用BSM模型估计的该期权的最接近价值是多少？
    \end{question}
...
\begin{question}
    ...
    一位固定收益投资公司的风险分析师正在研究股票价格运动和回报，以更好地理解市场动态。分析师特别感兴趣的是这些运动如何在Black-Scholes-Merton（BSM）模型中描述。哪一项陈述正确描述了这些运动？
    \end{question}
    ...
    股票价格服从对数正态分布，它们的收益服从正态分布，导致短期收益服从正态分布。
    ...
    在BSM模型中，短期股票收益服从均值为\(\mu\)和标准差为\(\sigma\)的正态分布，在较长时间段内，股票价格也服从正态分布。
    ...
    在BSM模型中，短期股票收益服从均值为\(\mu \Delta t\)和标准差为\(\sigma \sqrt{\Delta t}\)的正态分布，在较长时间段内，股票价格服从对数正态分布。
    ...
    股票价格的波动性在短期和长期内都是恒定的，股票价格总是服从正态分布。
    \end{choices}
...
\begin{question}
    ...
    \\一位投资公司的财务分析师正在评估股票期权定价。股票的周波动率为1.5\%。现在，你需要计算股票的年化波动率。假设一年有252个交易日，一年有52周。根据这些信息，计算股票的年化波动率。
    \end{question}
...
\begin{question}
    ...
    \end{itemize}
    一家公司有1亿股，每股价值40美元。公司正在考虑发行20万份认股权证。每份认股权证赋予持有人在5年后以60美元的价格购买一股股票的权利。5年期欧式看涨期权在股票上的价值为7.04美元。计算每份认股权证的价值。
    \begin{itemize}
    \item \( N = 1,000,000 \)（手中股票数量）
    \item \( M = 200,000 \)（新发认股权证数量）
    \item 5年期欧式看涨期权在股票上的价值为7.04美元。
    \end{itemize}
\end{question}
...
\begin{question}
    ...
    一位金融分析师正在评估公司发行的金融工具，并需要理解期权和权证之间的关键差异。哪一项陈述最准确地描述了期权和权证之间主要差异？
    \end{question}
...
\begin{question}
    ...
    一位投资银行的风险经理正在分析不支付股息的股票的美式和欧式看涨期权的差异。经理需要确定在什么条件下这些期权可能具有不同的价值。哪一项陈述是正确的？
    \end{question}
    ...
    对于不支付股息的股票，美式和欧式看涨期权的价值没有区别，因为未行使期权的价值总是高于行使期权的价值。
    ...
    美式看涨期权在派发股息时应该总是提前行使，因为这样可以立即收到股息。
...
\begin{question}
    ...
    一位衍生品交易员在衍生品交易台正在评估看涨期权空头头寸的风险。在这个上下文中，风险被定义为意外损失的可能性。在分析这些风险时，哪一项陈述是正确的？
\end{question}
...
\begin{choices}
    ...
    D（德尔塔）：衡量标的资产价格变化对期权价值的影响；正值表示随着价格上升，期权价值增加。
    ...
    V（维加）：衡量波动率变化对期权价值的影响；它在所有虚实程度级别的期权中表现出一致的敏感度。
    ...
    G（伽马）：衡量德尔塔对标的资产价格变动的敏感度，随着期权接近到期，其值相对稳定。
    ...
    T（西塔）：衡量时间流逝对期权价值的影响；通常不被视为风险因素。
\end{choices}
...
\begin{question}
    ...
    一位交易员正在评估看涨期权的价值。期权的到期时间为2年，当前的无风险连续利率为4\%，连续股息收益率为1\%。此外，交易员知道\( N(d_1) = 0.64 \)。该期权的德尔塔是多少？
\end{question}
...
\begin{question}
    ...
    一位投资顾问正在为客户设计一个投资策略，其中包括在标普500指数上写入大量看跌期权。由于期权的执行价格和到期时间在市场上不可用，顾问需要实施动态对冲策略来管理风险。哪一项关于这种策略风险的陈述是不正确的？
\end{question}

\begin{choices}
    \item The advisor can make the portfolio delta neutral by shorting index futures contracts.\\
    顾问可以通过做空指数期货合约使投资组合达到德尔塔中性。
...
\begin{question}
    ...
    \\组合经理John持有100份期权合约，其希腊字母如下：
    \begin{itemize}
        \item theta = +30,000; 
        \item vega = +250; 
        \item gamma = -150; 
    \end{itemize}
    即：正的theta、正的vega和负的gamma。哪一项额外交易，利用大致平值（ATM）期权，将对theta、vega和gamma进行中性化（对冲）？
\end{question}
...
\begin{question}
    ...
    \\组合经理持有以下不同期权合约的头寸：
    \begin{itemize}
        \item 在50,000份看涨期权合约中，每份期权有delta、vega和gamma分别为0.46、3.3和0.13。
        \item 在20,000份看涨期权合约中，每份期权有delta、vega和gamma分别为0.33、4.2和0.15。
        \item 在30,000份看跌期权合约的空头头寸中，每份期权有delta、vega和gamma分别为-0.54、3.0和-0.08。
    \end{itemize}
    该投资组合的整体delta、vega和gamma是多少？
\end{question}

\begin{choices}
    \item Delta = 32,600; Vega = 300; Gamma = 1,100\\
    德尔塔 = 32,600；维加 = 300；伽马 = 1,100
    \item Delta = 32,000; Vega = 320; Gamma = 1,200\\
    德尔塔 = 32,000；维加 = 320；伽马 = 1,200
    \item Delta = 32,400; Vega = 350; Gamma = 1,100\\
    德尔塔 = 32,400；维加 = 350；伽马 = 1,100
    \item Delta = 32,800; Vega = 310; Gamma = 1,150\\
    德尔塔 = 32,800；维加 = 310；伽马 = 1,150
\end{choices}
...
\begin{question}
    ...
    一位期权交易员正在评估各种期权希腊字母的特性，以优化他们的交易策略。哪一项关于期权希腊字母的陈述是正确的？
\end{question}
...
\begin{question}
    ...
    一位期权交易员目前持有2,000份欧式看涨期权合约的空头头寸，这些合约将在一个月内到期。当前股票价格为\$19，这些看涨期权的执行价格为\$20。交易员正在考虑基于简单一步二叉树模型的德尔塔对冲策略。在模型设置中，一个月后，股票价格将为\$23.75或\$15.2。交易员应该怎么做才能使头寸达到德尔塔中性？
\end{question}
...
\begin{question}
    ...
    \\一位交易员管理一个对隐含波动率增加高度敏感的投资组合，并且随着时间的推移遭受重大损失。交易员最可能采用哪种策略来对冲这个投资组合？
\end{question}
...
\begin{question}
    ...
    \\一位衍生品交易员在分析一个由看涨和看跌期权空头头寸组成的组合。组合具有以下特征：
    \begin{itemize}
        \item 组合是德尔塔中性的
        \item 所有期权大致是平值的（ATM）
        \item 所有期权都有接近到期的到期时间
    \end{itemize}
    哪一项最准确地描述了组合的西塔特性？
    \end{question}
...
一位组合经理正在分析各种欧式期权的敏感度（希腊字母）。哪一项关于期权希腊字母的陈述是正确的？
\end{question}
...
\item The sensitivity to volatility changes (vega) typically exhibits its highest values for deep in-the-money European options due to their substantial intrinsic value\\
    波动率变化（维加）对深度实值欧式期权的影响通常最大，因为这些期权具有较大的内在价值
\item For a European put option, its delta maintains significant negative values even as the underlying asset price increases substantially above the strike price\\
    对于欧式看跌期权，其德尔塔即使在标的资产价格大幅高于执行价格时也保持显著的负值
\item The rate of change in delta (gamma) tends to decrease as the time to expiration shortens for at-the-money options due to reduced time value\\
    德尔塔的变化率（伽马）随着到期时间缩短而减小，因为时间价值减少
\item For European call options with the same strike price and maturity, the time decay (theta) has a more negative value for at-the-money options than for out-of-the-money options\\
    对于相同执行价格和到期日的欧式看涨期权，平价期权的西塔比虚值期权的西塔有更大的负值
\end{choices}
...
\begin{question}
    ...
    \\一位对冲基金的期权交易员正在评估四种不同的投资组合风险状况。每个组合结合了不同的期权策略，导致不同的伽马和德尔塔暴露。鉴于市场波动预计在未来几周内显著增加，哪一种组合将使基金面临最高的风险水平？
    \end{question}
    ...
    \begin{question}
        ...
        \\一位衍生品交易员在同一标的股票上建立了以下头寸（当前价格\$50，所有期权到期时间为3个月）：买入80张平价看涨期权，德尔塔为0.5；卖出120张平价看跌期权，德尔塔为-0.5；卖出40张平价看涨期权，德尔塔为0.5。哪一项交易将使整个头寸达到德尔塔中性？
        \end{question}
        ...
        \begin{question}
            ...
            \begin{question}
                ...
                \\一位交易员卖出100万份欧式看涨期权，每份期权价格为\$100，执行价格为\$105，无风险利率为4\%，股票波动率为25\%。期权有一年的到期时间。根据布莱克-斯科尔斯-默顿模型，期权的理论价格为\$9.56每股。交易员的总收入为\$10万，理论利润为\$440,000。交易员使用止损策略进行风险管理。
                关于止损策略的有效性，哪一项陈述是正确的？
                \end{question}
                ...
                止损策略通过在价格超过\$105时购买股票，在价格低于\$105时卖出期权来获利，限制潜在损失。
                ...
                止损策略的有效性取决于股票价格变动，在高波动性期间可能并不总是有效。
                \end{choices}
...
\begin{question}
    ...
    在金融衍生品交易中，德尔塔衡量期权价格对标的资产价格变化的敏感度。基于德尔塔的特性，哪一项关于不同期权头寸的描述是正确的？
\end{question}
...
\begin{choices}
    ...
    做多看涨期权时，德尔塔为负值，基础资产价格上涨会导致期权价格下跌。
    ...
    做空看跌期权时，德尔塔为负值，基础资产价格下跌会导致期权价格上涨。
    ...
    做多看跌期权时，德尔塔为正值，基础资产价格下跌会导致期权价格下跌。
    ...
    \item When shorting a call option, Delta is negative, and an increase in the underlying asset's price leads to a decrease in the option's price.\\
    做空看涨期权时，德尔塔为负值，基础资产价格上涨会导致期权价格下跌。
\end{choices}
...
\begin{question}
    ...
    一位对冲基金的期权交易员正在评估其投资组合对市场波动变化的敏感性。他们需要了解维加如何影响期权定价，以便有效地管理风险。基于维加的关键特征，哪一项描述是正确的？
\end{question}
...
\begin{choices}
    ...
    更高的维加值意味着期权价格对波动率变化的敏感度较低，因此对冲频率可以降低。
    ...
    更低的维加值意味着期权价格对波动率变化的敏感度较高，因此需要更频繁的对冲。
...
\begin{question}
    ...
    一位固定收益投资公司的风险分析师正在评估时间衰减对长期期权头寸的影响。他们需要了解西塔如何影响这些头寸。哪一项描述最准确地反映了西塔对长期期权头寸的影响？
\end{question}
...
\begin{choices}
    ...
    长期期权的西塔通常为正值，因为期权的时间价值随着时间的推移而增加，导致期权价格上升。
    ...
    长期期权的西塔通常为负值，因为期权的时间价值随着时间的推移而减少，导致期权价格下降。
    ...
    西塔值与期权的伽马无关，因为西塔只衡量时间衰减对期权价格的影响，而伽马衡量德尔塔对标的资产价格变化的敏感度。
    ...
    当期权接近到期时，西塔值显著增加，因为期权的时间价值迅速减少，导致期权价格迅速下降。
\end{choices}
...
\begin{question}
    ...
    一位初级衍生品交易员在审查期权定价模型中的希腊字母定义，以更好地理解它们对期权策略的影响。基于希腊字母的定义，哪一项描述是正确的？
\end{question}
...
\begin{choices}
    ...
    德尔塔衡量期权价格对标的资产价格变化的敏感度，而伽马衡量期权价格对波动率变化的敏感度。
    ...
    维加衡量期权价格对无风险利率变化的敏感度，而rho衡量期权价格对标的资产价格变化的敏感度。
    ...
    西塔衡量期权价格对时间流逝的敏感度，而维加衡量期权价格对波动率变化的敏感度。
    ...
    伽马衡量期权价格对时间流逝的敏感度，而西塔衡量期权价格对标的资产价格变化的敏感度。
\end{choices}
...
\begin{question}
    ...
    一位组合经理正在考虑使用看跌期权来保护其投资组合免受市场下跌的影响，同时保持潜在收益。德尔塔对冲是实现这一保护的关键技术，涉及创建一个与所需期权德尔塔相等的反向头寸。基于德尔塔对冲的原则，哪一项描述是正确的？
\end{question}
...
\begin{choices}
    ...
    如果资产经理想要购买德尔塔为-0.236的看跌期权保护，她应该卖出投资组合的23.6\%。
    ...
    如果资产经理想要购买德尔塔为-0.236的看跌期权保护，她应该买入投资组合的23.6\%。
    ...
    为了匹配德尔塔，资产经理应该创建一个与所需期权德尔塔相等的反向头寸使用标的资产。
    ...
    为了匹配德尔塔，资产经理应该创建一个正向头寸使用指数期货等于所需期权德尔塔。
\end{choices}
...
\begin{center}
    ...
    \end{tabular}
\end{center}
    Ken管理一个德尔塔中性、200维加、30伽马的投资组合。有两个期权可供选择。根据以下表格中的信息，估计（a）在期权A和标的资产上应采取什么头寸以实现维加和德尔塔中性，以及（b）在期权B和标的资产上应采取什么头寸以实现伽马和德尔塔中性。
    \begin{center}
        \begin{tabular}{|c|c|c|c|}
            \hline
            & Delta & Vega & Gamma \\
