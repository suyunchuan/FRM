...
\begin{question}
    ...
    一位对冲基金的风险分析师正在研究投资组合的收益率。她需要计算各种投资组合及其组合的期望收益率。为了确保她的分析正确，她回顾了期望算子\(E[\cdot]\)的基本性质。哪一项关于期望算子\(E[\cdot]\)的性质的陈述是正确的？
\end{question}
...
\begin{choices}
    ...
    对于任何非零投资组合收益率\(X\)，\(E[1/X] = 1/E[X]\)，这表明期望算子适用于收益率的非线性函数。
\end{choices}
...
\begin{question}
    ...
    一位金融公司的风险分析师正在研究概率函数。令\(g(x)\)表示概率函数（离散随机变量的概率质量函数，PMF，和连续随机变量的概率密度函数，PDF），令\(G(x)\)表示相应的累积分布函数（CDF）。对于连续变量，\(G(X)\)是PDF的积分（或反导数）。哪一项关于这些概率函数的陈述是错误的？
    \end{question}
    
    \begin{choices}
        \item For continuous random variables, the probability of a single point is zero; for example, \(\text{Pr}[S = - 2.50\%]=0\).\\
        对于连续随机变量，单个点的概率为零；例如，\(\text{Pr}[S = - 2.50\%]=0\)。
        ...
        任何随机变量的累积分布函数（CDF），无论是离散还是连续，都有一个下界为零和上界为1；即，\(G(-\infty)=0\)和\(G(+\infty)=1.0\)。
        ...
        贝叶斯定理可以应用于离散和连续随机变量，而不需要将连续变量转换为离散变量。
        ...
        离散随机变量的累积分布函数可能有平坦区域，而连续随机变量的累积分布函数是严格递增的。
    \end{choices}
...
一位金融机构的风险分析师正在评估一个操作流程。一年期内重大损失事件的频率范围从0到6.0，由以下离散概率质量函数（pmf）描述，该函数表示完整的概率分布，其中\(k\)是一个常数。
\begin{center}
\begin{tabular}{|c|c|c|c|c|c|c|c|}
\hline
损失事件 (\(X\)) & 0 & 1 & 2 & 3 & 4 & 5 & 6 \\
\hline
概率质量函数 (\(f(x)\)) & \(15k\) & \(8k\) & \(6k\) & \(4k\) & \(3k\) & \(2k\) & \(1k\) \\
\hline
\end{tabular}
\end{center}
明年发生少于三次重大损失事件的近似概率是多少？\\
\end{question}
...
\begin{question}
    ...
    一位债券交易公司的风险分析师正在评估一个面值为\$12.00的零息债券。该债券的价格\(x\)的概率密度函数为\(f(x)=\frac{x}{10}-0.6\)，在区间\([6, 10]\)内。债券价格在\$7.50和\$8.50之间的概率是多少？
    \end{question}
...
一位投资银行的初级风险分析师正在研究两个离散随机变量 \(X\) 和 \(Y\) 的以下联合分布。例如，\(P(X = 5,Y = 25)=0.08\)。
\begin{center}
    \begin{tabular}{|c|c|c|c|c|}
    \hline
    \multirow{2}{*}{X的值} & \multicolumn{4}{|c|}{Y的值} \\
    \cline{2-5}
    & 15 & 25 & 35 & 45 \\
    \hline
    2 & 0.05 & 0.15 & 0.07 & 0.05 \\
    ...
    \end{tabular}
    \end{center}
给定 \(X = 8\) 时，\(Y\) 的条件标准差是多少？即 \(SD(Y|X = 8)\)？ \\
\end{question}
...
\begin{question}
    ...
    一位金融研究公司的初级分析师正在研究两个随机变量 \(X = \{3, 8, \text{ 或 } 13\}\) 和 \(Y=\{2, 4, \text{ 或 } 6\}\)。以下概率矩阵显示了联合概率：
    \begin{center}
        \begin{tabular}{|c|c|c|c|}
        \hline
        & \multicolumn{3}{|c|}{Y} \\
        ...
        \end{tabular}
    \end{center}
    给定 \(X\) 和 \(Y\) 是独立的，\(X\) 和 \(Y\) 的乘积的期望值是多少，即 \(E(XY)\)？\\
    \end{question}
...
\begin{question}
    ...
    一位资产管理公司的风险分析师正在分析四个不同随机变量的累积分布函数（CDF）。每个随机变量只能取值在-10和10之间。每个图中虚线垂直线表示分布的均值。哪一种分布的方差最高？
    \end{question}
    
    \begin{choices}
        \item \includegraphics[width=0.2\textwidth]{images/CDF-1.png}
        \item \includegraphics[width=0.2\textwidth]{images/CDF-2.png}
        \item \includegraphics[width=0.2\textwidth]{images/CDF-3.png}
        \item \includegraphics[width=0.2\textwidth]{images/CDF-4.png}
    \end{choices}
...
\begin{question}
    ...
    一位全球宏观投资基金的风险经理正在评估国际投资组合经理的表现。被选中的这些经理的年化总回报率为23\%，19\%，13\%，20\%，17\%。这些回报数据的适当无偏样本方差是多少？
    \end{question}
...
\begin{question}
    ...
    一位大型资产管理公司的金融分析师正在研究两个投资组合在同一时间段内的收益分布。该分析师收集了以下数据：
    \begin{center}
    \begin{tabular}{|c|c|c|}
    \hline
    投资组合 & 偏度 & 峰度 \\
    ...
    \end{tabular}
    \end{center}
    该分析师声称投资组合C的分布比正态分布更尖，而投资组合D的分布在左侧有长尾。下列关于这些陈述的陈述中，哪一项是正确的？
    \end{question}
...
\begin{question}
    ...
    一位投资银行的风险分析师正在比较不同的概率分布，以评估变量超过指定极端值的可能性。给定所有分布具有相同的均值和方差，且极端值\(X\)大于均值，哪一种分布将使变量超过\(X\)的概率最低？
    \end{question}
    ...
    \begin{question}
        ...
        一位研究公司的金融分析师正在分析特定公司在某一时期内的收益率分布。经过一些计算，分析师发现中位数收益率为4.6\%，均值收益率为4.1\%，众数收益率为4.9\%。此外，计算出的峰度为2.2。基于这些信息，哪一项正确地描述了收益率随时间变化的分布？
        \end{question}
...
\begin{question}
    ...
    一位风险分析师Lily，FRM，正在为一家咨询公司工作。她被一家客户公司委托评估一项股权投资，并创建了一个连续分布来评估其回报表现。Lily计算了以下统计数据：
    \begin{itemize}
    \item 均值：8.2\%
    \item 中位数：5.5\%
    \item 超额峰度：2.785
    \end{itemize}
    哪一项关于偏度和峰度的陈述最可能是正确的？
    \end{question}
...
\begin{question}
    ...
    一位资产管理公司的风险分析师正在评估特定投资基金的回报分布。该回报分布的均值和方差与正态分布相同。然而，它在两侧都有大量数据点，并且右侧尾部有更多的观测值而不是左侧尾部。哪一项陈述准确地描述了这个分布？
    \end{question}
    ...
    \begin{question}
        ...
        一位股票对冲基金的风险分析师正在评估月度股票收益数据的特征。该分析师使用一个大小为380的样本。获得了以下汇总统计量：
        \begin{center}
        \begin{tabular}{|c|c|}
        \hline
        统计量 & 月度收益率 \\
...
\begin{question}
    ...
    一位银行的风险分析师知道真实收益率分布是尖峰分布。在假设正态分布的情况下计算价值风险（VaR）时，哪一项陈述是正确的？
    \end{question}
    ...
    \checklist{分布的尾部特征（理解厚尾分布与正态分布在极端事件概率上的差异）}
        ...
        \begin{question}
            ...
            一位量化投资公司的分析师正在研究两个随机变量之间的关系。给定一个随机变量\(X\)遵循正态分布，且\(Y = C + D X\)，其中\(C\)和\(D\)是常数，\(D\)是负数。哪一项陈述是正确的？
        \end{question}
...
\begin{question}
    ...
    一位金融集团的风险分析师Amy，FRM，正在分析一个金融产品。她将该产品的收益率表示为\(X\)。经过收集市场数据，她计算了\(X\)的四个命名矩：
    \begin{center}
    \begin{tabular}{|l|l|}
    \hline
    矩 & 值 \\
    ...
    \end{tabular}
    \end{center}
    巧合的是，她发现另一个金融产品\(Y\)，其收益率与\(X\)有完美的线性关系\(Y = - 4X+15\)。哪一项关于\(Y\)的陈述是不正确的？
    \end{question}
...
\begin{question}
    ...
    \begin{choices}
        ...
        四分位距（IQR）是衡量离散程度的一种指标，与标准差在对其敏感度上的差异。
    \end{choices}
...
\begin{question}
    ...
    一位全球投资银行的风险分析师正在分析各种金融工具的日收益率。她需要选择适当的描述性统计量来总结收益率分布，特别是在处理极端市场条件时。基于描述性统计量的定义和性质，哪一项陈述是正确的？
\end{question}
...
\begin{choices}
    ...
    算术平均数通过给予所有观测值相同权重，在收益率分析中提供了增强的稳定性，使其在市场压力期间特别可靠。
    ...
    算术平均数对异常值的敏感性使其成为风险评估的最佳测量方法，因为它能捕捉到市场波动影响的全部。
\end{choices}
...
\begin{question}
    ...
    一位全球投资银行的对冲基金交易员正在为一只波动性股票定价期权。她知道Jensen不等式对于理解为什么期权价格不同于其内在价值至关重要，特别是对于凸收益函数。基于Jensen不等式的定义和性质，哪一项陈述是不正确的？
\end{question}

\begin{choices}
    \item If \(h(X)\) is a convex function, then \(E[h(X)] > h(E[X])\).\\
    如果\(h(X)\)是凸函数，则\(E[h(X)] > h(E[X])\)。
    \item If \(g(X)\) is a concave function, then \(E[g(X)] < g(E[X])\).\\
    如果\(g(X)\)是凹函数，则\(E[g(X)] < g(E[X])\)。
...
一位对冲基金的组合经理正在分析两个资产在市场压力期间的行为。经理想要理解一个资产的极端运动如何影响另一个资产的回报，并正在检查高阶共矩来改进投资组合风险管理。哪一项关于共偏度和共峰度测量的陈述是正确的？
\end{question}
\begin{choices}
...
\begin{question}
    ...
    一位固定收益投资公司的风险分析师正在评估一个投资组合的违约风险。该投资组合由七个债券组成，每个债券的1年违约概率为13\%。这些债券的违约事件是独立的。在未来一年内，恰好有三个债券违约的概率是多少？
    \end{question}
...
\begin{question}
    ...
    一位金融顾问正在评估一个二项分布投资情景。在这个情景中，每次成功的概率为\(p = 0.07\)，试验次数为\(n = 25\)。与概率为30.0\%的逆累积分布函数相关的是多少？
    \end{question}
...
\begin{question}
    ...
    一位大型投资银行的风险分析师正在评估一个企业债券投资组合。该投资组合中的所有债券具有相同的年化违约概率，且这些债券的违约事件是独立的。这一年中违约债券数量的分布类型是什么？
    \end{question}
    ...
    \begin{question}
        ...
        一位金融研究公司的初级分析师正在研究概率分布及其收敛性质。哪一种分布在其中一个参数增加时不会趋近于正态分布？
        \end{question}
...
\begin{question}
    ...
    一位投资者正在参加一个金融知识的多项选择题测试。该测试有八个问题，每个问题有四个选项。如果投资者需要至少两个正确答案才能通过测试，那么投资者通过猜测的概率是多少？
    \end{question}
...
\begin{question}
    ...
    一位金融机构的风险分析师正在进行集中趋势和离散度分析。单日超额（违约）的概率为1.2\%。分析师测量了超过次数的分布在350个交易日内均值和标准差。超过次数的均值和标准差是多少？
    \end{question}
...
\begin{question}
    ...
    一位保险呼叫中心的服务经理知道呼叫中心每小时平均接到三个电话。在7小时内，呼叫中心接到24个电话的概率是多少？
    \end{question}
    ...
    \begin{question}
        ...
        一位金融机构的风险分析师正在评估一个低严重度行政流程的操作风险。这个流程通常每周平均产生12个错误（每周有5个工作日）。如果这个流程的损失频率遵循泊松分布，哪一个值最接近明天发生超过一个错误的概率；即，\(P(K > 1|\lambda=\frac{12}{5})\)?
        \end{question}
...
\begin{question}
    ...
    一位银行的操作风险经理正在使用泊松分布来估计未来两年内损失超过300万美元的频率。根据过去12年的历史数据，损失超过300万美元的平均频率为每年2次。假设过去的数据是未来事件的良好指示，并且所有损失都是独立的，那么在接下来的两年内，损失超过300万美元的概率最多为一次的概率是多少？
    \end{question}
...
\begin{question}
    ...
    一位金融机构的风险经理正在回测公司对价值风险（VaR）的模型。在过去的180个交易日中，经理观察到6次违约（exceedances）。假设模型正确校准，并且所有违约都是相互独立的，那么在300个交易日中，恰好有8次违约的概率是多少？
    \end{question}
...
\begin{question}
    ...
    一位风险分析师正在处理随机变量。给定\(X\)是一个遵循泊松分布的随机变量，\(\lambda = 3\)。定义一个新的随机变量\(Y = 2 + 3X\)。\(Y\)的期望和方差是多少？
    \end{question}
...
\begin{question}
    ...
    一位制造公司的风险分析师正在使用公司专有的预测模型。该模型假设随机变量遵循泊松分布。分析师正在评估另一个公司的装配生产过程中缺陷数量的概率。已知每次生产运行出现缺陷的概率为0.008，那么在800次生产运行中，恰好有5次缺陷的概率是多少？
    \end{question}
    ...
    \begin{question}
        ...
        一位固定收益投资公司的风险分析师正在评估五个不同的随机变量以进行风险评估。这五个随机变量如下：
        \begin{itemize}
        \item 标准正态随机变量；无需参数。
        \item 自由度为12的学生t分布；df = 12。
        \item 表示贷款违约概率的伯努利变量，其中违约概率（PD）= 6\%；p = 0.06。
        \item 模拟交易日内系统故障频率的泊松分布，其中 \(\lambda=7.0\)。
        \item 描述60只债券投资组合中违约数量的二项变量，每只债券的PD = 3\%；n = 60，p = 0.03。
        \end{itemize}
        上述随机变量中，方差最低值和最高值分别是哪一个？\\
        \end{question}
...
\begin{question}
    ...
    一位投资银行的风险分析师正在评估一个由五只债券组成的投资组合。每只债券的1年违约概率为22\%。这些债券的违约事件是独立的。在未来一年内，违约债券数量的均值和方差是多少？
    \end{question}
...
\begin{question}
    ...
    一位风险分析师知道变量\(Y\)是一个正态随机变量，\(Y\sim N(\mu,\sigma^{2})\)。哪一个\(X\)变量是对数正态分布的？
    \end{question}
    ...
    \begin{question}
        ...
        给定两个独立变量\( X \)和\( Y \)，它们服从对数正态分布，哪一项陈述关于乘积\( Z = X \times Y \)的分布是正确的？
        \end{question}
...
\begin{question}
    ...
    一位金融分析师使用35个观测值（\(n = 35\)）进行多元回归。回归结果包括五个回归系数，包括截距项和四个偏回归系数。OLS估计遵循学生t分布。这个学生t分布的方差和峰度分别是多少？
    \end{question}
...
\begin{question}
    ...
    一位风险分析师正在比较正态分布和学生t分布，它们具有相同的均值和标准差。哪一项陈述是正确的？
    \end{question}
...
\begin{question}
    ...
    一位资产管理公司的金融分析师正在评估统计分布在回归模型假设检验中的使用。以下关于卡方分布和F分布的陈述中，哪一项是正确的？
    \end{question}
    ...
    \begin{choices}
        ...
        给定一个计算出的F比率，其中\(F=\frac{(ESS/df_1)}{(SSR/df_2)}\)，加上样本量\(n\)和独立变量个数\(k\)，可以在多元回归中计算调整的决定系数\(\bar{R}^2\)。
        ...
        F分布用于检验多元回归中的所有斜率系数是否同时为零，确定整体回归模型是否显著。
        ...
        随着自由度增加，卡方分布趋近于正态分布，而F分布趋近于对数正态分布。
        ...
        卡方分布用于检验样本方差是否来自具有特定方差的总体。即，给定一个观察到的样本方差，真总体方差是否不同于假设值？
    \end{choices}
...
\begin{question}
    ...
    一位固定收益投资公司的风险分析师正在评估分布。在卡方分布和F分布方面，以下哪一项是正确的，除了哪一项是错误的？
    \end{question}
...
\begin{question}
    ...
    一位知名的投资银行初级分析师被要求向金融学本科生介绍常用的金融分布。在准备过程中，分析师识别了一些常见的误解概念，并计划在演讲中包括一个简短的测验。以下哪一项陈述最可能正确？
    \end{question}
    ...
    \begin{choices}
        ...
        对数正态分布、卡方分布和F分布的概率密度函数图像都是左偏态，且都以零为下界。
    ...
    \begin{question}
        ...
        \begin{question}
            ...
            一位对冲基金的量化分析师正在研究t分布用于模拟资产回报，特别是在市场压力时期，回报通常具有厚的尾部。分析师正在审查t分布的概率密度函数（PDF）的不同自由度（df）的性质。根据t分布的定义和性质，以下哪一项陈述是不正确的？
        \end{question}
        
        \begin{choices}
            \item The probability density function of t-distribution is parameterized by its degrees of freedom \(df\), where \(df = n-1\).\\
            t分布的概率密度函数由其自由度 \(df\) 参数化，其中 \(df = n-1\)。
            ...
            t分布的概率密度函数形状为钟形，且关于y轴对称，相比正态分布，t分布具有更厚的尾部，这使其更适合建模金融资产收益率的极端值。
...
The portfolio value is defined as \(Y = W \cdot X_1 + (1-W) \cdot X_2\). What is the expected value of Y?\\
    ...
    一位对冲基金的组合经理正在评估一个动态投资策略，该策略结合了两种具有不同分布的资产。策略涉及：
    \begin{itemize}
        \item 资产1遵循均匀分布：\(X_1 \sim U(5,10)\)
        \item 资产2遵循均匀分布：\(X_2 \sim U(11,20)\)
        \item 切换参数 \(W\) 遵循伯努利分布，其中 \(p = 0.4\)
    \end{itemize}
    组合价值定义为 \(Y = W \cdot X_1 + (1-W) \cdot X_2\)。Y的期望值是多少？
\end{question}
...
\begin{question}
    ...
    Emma Davis是一位大型国际银行的风险经理，她正在使用双因子模型评估市场C和市场D：
    \[R_j=\alpha_j+\beta_{j,1}F_1+\beta_{j,2}F_2+\epsilon_j\]
    其中 \(R_j\) 是资产 \(j\) 的收益率，\(\beta\) 是因子敏感性，\(F\) 是因子。随机误差 \(\epsilon_j\) 的均值为零，且与因子以及其他资产收益的随机误差不相关。为了计算市场C和市场D之间的协方差，Davis构建了以下全球资产的因子协方差矩阵：
    \begin{center}
        \begin{tabular}{|c|c|c|}
        \hline
        & 全球债券因子 & 全球股票因子 \\
        ...
        \end{tabular}
    \end{center}
    假设对全球股票因子的因子敏感性，市场C为0.7，市场D为0.95；对全球债券因子的因子敏感性，市场C为0.45，市场D为0.65。市场C和市场D之间的协方差最接近：\\
    \end{question}
...
\begin{question}
    ...
    一位投资公司的风险分析师正在分析两个随机变量\(X\)和\(Y\)，它们代表两个不同的投资组合的年回报。已知\(E[X]=5\)，\(E[Y]=6\)和\(E[XY]=31\)，协方差\(Cov[X, Y]\)是多少？
    \end{question}
...
\begin{question}
    ...
    \begin{question}
        ...
        一位金融顾问正在为客户构建一个投资组合。顾问计划将两只股票的年度回报联合正态分布且不相关。股票X的期望回报为8\%，回报的标准差为25\%；股票Y的期望回报为13\%，回报的标准差为18\%。如果顾问创建一个等权重投资组合，这两只股票，下一年的投资组合回报率大于13\%的概率是多少？
        \end{question}
...
\begin{question}
    ...
    一位金融顾问正在分析两个基金的潜在合并。宁静基金价值8000万美元，其最近表现不佳。顾问建议将其与动态基金合并，动态基金价值3000万美元。宁静基金的回报服从正态分布，均值为4\%，标准差为8\%；动态基金的回报服从正态分布，均值为9\%，标准差为18\%。高级管理层要求顾问估计合并投资组合的回报率超过28\%的可能性。假设两个基金的回报是独立的，合并基金的回报率超过32\%的概率最接近的估计是多少？
    \end{question}
...
\begin{question}
    ...
    \end{center}
        ...
        一位投资者拥有一个由股票C和股票D组成的组合。当前价值、估计的年期望回报和估计的年回报标准差如下表所示：
        \begin{center}
        \begin{tabular}{|c|c|c|}
        \hline
        & 股票C & 股票D \\
        \hline
        当前价值 (美元) & 30,000 & 70,000 \\
        \hline
        期望回报 & 7\% & 10\% \\
        \hline
        标准差 & 18\% & 22\% \\
        ...
        \end{tabular}
        \end{center}
        如果股票C和股票D的回报的相关系数为0.4，则今年年底投资组合的预期价值，在两个标准差范围内，将在：
    \end{question}
...
\begin{question}
    ...
    一位金融顾问已确定明年三种潜在经济状态的概率：扩张、稳定和收缩。一位投资分析师已估计了两种股票M和N在这三种情景下可能的收益率，如下表所示：
    \begin{center}
    \begin{tabular}{|c|c|c|c|}
    \hline
    状态 & 概率 & 股票M的收益率 & 股票N的收益率 \\
    \hline
    扩张 & 0.25 & 0.35 & 0.25 \\
    \hline
    稳定 & 0.55 & 0.15 & 0.15 \\
    ...
    \end{tabular}
    \end{center}
    给定股票M和N的估计收益率的标准差分别为18.0\%和11.0\%，股票M和N的估计收益率的协方差是多少？\\
    \end{question}
...
Given that \(X\) and \(Y\) are random variables, and \(m\), \(n\), \(p\), and \(q\) are constants, which of the following statements is most likely incorrect?\\
    给定\(X\)和\(Y\)是随机变量，\(m\)，\(n\)，\(p\)和\(q\)是常数，哪一项陈述最可能不正确？
\end{question}
...
\begin{choices}
    ...
    如果\(n<0\)且\(q<0\)，则\(Cov(m + nX,p + qY)=Cov(X,Y)\)。
    \item \(Corr(m + nX,p + qY)=Corr(X,Y)\), if \(n<0\) and \(q<0\)\\
    如果\(n<0\)且\(q<0\)，则\(Corr(m + nX,p + qY)=Corr(X,Y)\)。
\end{choices}
...
\begin{question}
    ...
    一位金融机构的风险分析师正在研究涉及多个独立随机变量的模型。在考虑这些独立随机变量的乘积的特征函数时，它等于：
    \end{question}
...
\begin{question}
    ...
    一位量化交易公司的风险分析师正在评估一个由随机变量\(X\)和\(Y\)表示的两个资产组成的组合。每个\(X\)和\(Y\)服从标准正态分布，且\(X\)和\(Y\)的协方差为0.15。组合价值\(Z = 6X+3Y\)的方差是多少？
    \end{question}
...
\begin{question}
    ...
    \end{tabular}
    \end{center}
    一位风险分析师正在检查一个联合概率矩阵，该矩阵显示了通胀（有下降、稳定和上升三种状态）和市场（有熊市、区间震荡和牛市三种状态）之间的关系。以下关于这个联合概率矩阵的陈述中，哪一项是错误的？
    \begin{center}
        \begin{tabular}{|c|c|c|c|c|}
        \hline 
        \multirow{2}{*}{通胀(i)}& \multicolumn{3}{|c|}{市场(M)} &\multirow{2}{*}{ }\\ 
        \cline{2 - 4} 
        & 熊市 & 区间震荡 & 牛市 &  \\ 
        \hline 
        下降 & 4\% & 7\% & 6\% & 17\% \\ 
    ...
    \begin{question}
        ...
        \end{tabular}
        \end{center}
        一位股票对冲基金的风险分析师正在分析下表中两个离散随机变量 \(X\) 和 \(Y\) 的联合分布。给定 \(X=7\)，该分析师旨在确定 \(Y\) 的条件标准差。以下哪个选项是正确的？
        \begin{center}
        \begin{tabular}{|c|c|c|c|c|}
        \hline 
        \multirow{2}{*}{X}& \multicolumn{4}{|c|}{Y} \\ 
...
\begin{question}
    ...
    一位股票对冲基金的风险分析师正在分析随机变量$X$和$Y$的联合概率分布，其分布为$f(x,y)=k\times x\times y$，其中$x = 1,2,4$，$y = 1,2,4$，$k$是一个正的常数。$X + Y$超过6的概率是多少？
    \end{question}
...
\begin{question}
    ...
    一位共同基金的组合经理正在分析不同股票对之间的联动性。她正在比较两对股票：股票A和B是小盘成长股，在科技行业，波动性高；股票C和D是大盘价值股，在传统行业，波动性低。哪一项陈述最适用于比较这些对之间的联动性强度？
\end{question}
...
\begin{choices}
    ...
    股票A和B之间的协方差必须高于股票C和D之间的协方差，因此它们的联动性更强。
    ...
    股票A和B之间的相关系数必须高于股票C和D之间的相关系数，因为它们是小盘股。
    ...
    为了准确比较股票A和B与股票C和D之间的联动性，应该计算并比较它们的相关系数。
    ...
    股票C和D之间的协方差必须低于股票A和B之间的协方差，因为它们的回报波动性较低。
\end{choices}
...
\begin{question}
    ...
    一位共同基金的组合经理正在分析添加多个股票到她的投资组合的风险分散效果。她特别感兴趣的是理解独立且同分布（i.i.d.）回报的统计特性如何影响投资组合的整体风险特征。根据i.i.d.随机变量的定义和性质，以下哪一项陈述是不正确的？
\end{question}
...
\begin{choices}
    ...
    一个等权重投资组合，由\(n\)个i.i.d.股票组成，其期望回报为\(n\mu\)，其中\(\mu\)是每个股票的期望回报。
...
一位全球投资银行的风险分析师正在研究不同金融变量之间的关系。理解协方差和相关系数之间的关系对于风险评估至关重要。以下关于协方差和相关系数的关系的陈述中，哪一项是不正确的？
\end{question}

\begin{choices}
    \item If \(cov(A,B) > cov(B,C)\), then \(\rho(A,B) > \rho(B,C)\) must be true.\\
    如果\(cov(A,B) > cov(B,C)\)，则\(\rho(A,B) > \rho(B,C)\)必须为真。
    ...
    协方差的大小取决于变量的标准差，因此比较协方差需要考虑变量的尺度。
\end{choices}
...
一位投资银行的风险分析师正在研究中心极限定理（CLT），这是一个统计理论中的基本原理，描述了在某些条件下样本均值的分布特性。以下关于中心极限定理的陈述中，哪一项是不正确的？
\end{question}
...
\begin{choices}
    ...
    中心极限定理要求总体均值和方差必须有限，样本量应超过30，且抽样过程必须遵循简单随机抽样，不替换。
    ...
    根据中心极限定理，样本统计量\(\bar{X}\)的期望等于总体参数\(\mu\)，证明了样本均值的无偏性。
    ...
    样本统计量\(\bar{X}\)的方差等于\(\sigma^2\)，无论估计过程中使用的样本量如何，方差保持不变。
    ...
    中心极限定理指出，对于足够大的样本量，样本统计量\(\bar{X}\)的分布近似于正态分布，无论总体分布如何。
\end{choices}
...
\begin{question}
    ...
    一位金融分析公司的初级统计学家正在评估不同的估计量用于金融模型中的特定参数。如果一个估计量的抽样分布的方差小于所有其他无偏估计量的参数，这个估计量具有什么特性？
    \end{question}
...
一位对冲基金的组合经理正在分析基于大数定律和一致性的估计量的性质。以下哪一项陈述是不正确的？
\end{question}
...
\begin{question}
    ...
    一位专注于科技投资的初级风险分析师Lily被分配估算科技股票指数的波动率。她发现一个样本统计量的期望值等于总体波动率。此外，她发现增加样本量会减少这个统计量的抽样误差。这个统计量最好如何描述？
    \end{question}
...
\begin{question}
    ...
    一位投资公司的金融分析师正在分析某个工业部门的股票的市盈率（P/E）。这个部门40只股票的平均市盈率是22，样本标准差是4.2。最接近样本均值标准误差的值是多少？
    \end{question}
...
\begin{question}
    ...
    一位对冲基金的量化分析师正在评估不同的统计量用于投资组合风险评估。她需要在标准差和标准误差之间选择，以描述她估计的不确定性。根据标准误差和标准差的定义和性质，以下哪一项陈述是不正确的？
\end{question}

\begin{choices}
    \item Standard error is the standard deviation of the mean, measuring the uncertainty of the estimator.\\
    标准误差是样本均值的标准差，用于衡量估计量的不确定性。
...
\begin{question}
    ...
    \begin{choices}
        ...
        所有其他条件相同，减少犯第一类错误的概率是以增加犯第二类错误的概率为代价的。
    \end{choices}
...
\begin{question}
    ...
    \begin{question}
        ...
        一位大型银行的风险分析师正在审查巴塞尔回测框架。根据巴塞尔回测框架指南，如果前一年有六个或更多例外，惩罚开始适用。这个测试的第一类错误率是13\%。如果真实覆盖率是96\%而不是要求的98\%，测试的功效是85\%。这意味着存在：
        \end{question}
...
\begin{question}
    ...
    一位金融咨询公司的初级分析师正在研究改变显著性水平对假设检验的影响。如果显著性水平从10\%降低到5\%，哪一项关于假设检验的陈述是正确的？
    \end{question}
...
\end{center}
 ...
 一位经纪公司的金融分析师正在分析一家名为Wilson Corp的公司的月度股票收益率。对于过去35个月股票收益率的样本，平均收益率为5\%，样本标准差为22\%。由于总体方差未知，样本的标准误估计为：
\[S_{\bar{x}}=\frac{22\%}{\sqrt{35}}=3.71\%\]

相关的t表值如下（\(t_{i,j\%}\) 表示自由度为 \(i\) 的t分布值的第 \((100 - j)\) 百分位数）：
\begin{center}
\begin{tabular}{|c|c|}
    \hline
    \(t_{34,2.5\%}\) & 2.032\\
...
\begin{question}
    ...
    一位金融分析师正在考虑一只初始价格为\(\$80\)的股票。一年后的价格由\(S = 80e^{r}\)给出，其中收益率\(r\)服从均值为\(0.15\)，标准差为\(0.25\)的正态分布。以\(99\%\)的置信度，四舍五入后，\(S\)将落在以下哪个范围内？
    \end{question}
...
\begin{question}
    ...
    一位金融机构的风险分析师正在进行假设检验。已知正态总体的标准差为12，假设的均值为10。抽取样本大小为144。在显著性水平为0.01时，样本均值的接受范围是多少？
    \end{question}
    ...
    \begin{question}
        ...
        \end{tabular}
        \end{center}
        一位名为Alex的金融公司风险分析师决定使用均值为$\mu = \$5.00$、标准差为$\sigma=\$2.00$的正态分布来模拟一家名为Green Leaf Corp的公司的每股收益（EPS）。该公司在任何给定时期内EPS位于$\$4.40$和$\$5.22$之间的概率是多少？
...
\begin{question}
    ...
    一位名为Emma的金融公司风险分析师正在评估一家名为AI Growth Pro的新股票基金。该基金由使用机器学习算法的AI管理。基金声称其真实（总体）波动率不超过12.0\%每年。随机抽取35个观测值得到样本波动率为14.0\%每年。在90\%的置信水平下，考虑这是一个单侧检验，原假设假设波动率等于或小于12.0\%每年，是否应该拒绝原假设，支持基金的真实波动率超过12.0\%？
    \end{question}
...
\begin{question}
    ...
    一位资产管理公司的风险分析师正在评估股票ABC的表现。使用过去15个月的收益率，该分析师估计股票ABC的平均月度收益率为-0.9\%，标准误为3.2\%。相关的单尾t分布表如下：
    \begin{center}
    \begin{tabular}{|c|c|c|c|}
    \hline
    自由度 & $\alpha = 0.10$ & $\alpha = 0.05$ & $\alpha = 0.025$ \\
    \hline
    12 & 1.356 & 1.782 & 2.179 \\
    \hline
    13 & 1.350 & 1.771 & 2.160 \\
    ...
    \begin{question}
        ...
        \end{tabular}
        \end{center}
        一位金融研究公司的风险分析师正在研究一只名为Alpha对冲基金的新对冲基金。该基金已运营2.5年。其平均月度收益率为7\%，标准差为6\%。该基金的目标是控制以月度收益率标准差衡量的波动率。该分析师被要求检验零假设：Alpha对冲基金月度收益率的波动率等于5\%，备择假设：波动率不等于5\%。假设所有月度收益率独立同分布且服从正态分布，并使用下表，应该使用哪种正确的检验方法？在5\%的显著性水平下，正确的结论是什么？
        t表：学生t分布的单尾概率的逆
        \begin{center}
        \begin{tabular}{|c|c|c|}
        \hline
        自由度 & 单尾概率 = 5.0\% & 单尾概率 = 2.5\% \\
        \hline
        29 & 1.699 & 2.045 \\
        \hline
        30 & 1.697 & 2.042 \\
        ...
        \begin{question}
            ...
            一位投资银行的一名初级分析师正在研究某只股票的回报。使用71个观测值，分析师确定该股票的回报标准差为24\%。分析师想要检验回报标准差是否高于16\%，在0.05显著性水平下。已知自由度为70，单尾概率为0.05，临界值为90.53，分析师应该得出什么结论？
            \end{question}
...
一位大型金融机构的风险分析师正在检验某个变量的总体均值小于或等于50的原假设。从4,000,000个个体中随机抽取100个观测值。样本均值为51.8，样本标准差为6。对于总体均值的检验，最合适的检验统计量是什么，在5\%显著性水平下，正确的结论是什么？
\end{question}
...
\begin{question}
    ...
    一位风险分析师从非常大的数据集中估计了正态分布的参数。分析师使用Z值1.96构建一个双侧置信区间，基于估计的分布参数。关于置信区间，哪一项陈述最可能正确？
    \end{question}
...
一位对冲基金的组合经理正在使用假设检验评估基金经理的表现。经理想要检验基金的表现是否优于基准。关于单侧假设检验，哪一项陈述是不正确的？
\end{question}
...
一位对冲基金的量化研究员正在对交易策略进行多次假设检验。关于多次检验及其控制方法，哪一项陈述是不正确的？
\end{question}
...
\begin{choices}
    ...
    多次检验增加了第一类错误的概率，因为同一个样本被用于测试多个假设。
    ...
    当n = 100，独立检验和5\%显著性水平下，至少找到一个显著优于市场的策略的概率为99.4\%。
    ...
    如果你连续在同一数据集上测试策略，你最终会找到一个看似有效的策略。如果你随后在不同数据上测试这个策略，它很可能仍然有效。
    ...
    显著性水平\(\alpha\)（错误拒绝真结果的概率）仅对单个假设检验是准确的，但在多次检验中不适用。
\end{choices}
...
一位全球投资银行的量化分析师对一只股票的日收益率与标普500指数收益率进行普通最小二乘回归分析。该分析使用了750个交易日的数据。回归结果总结如下表：
...
预测变量 & 系数 & 标准误 & t统计量 & p值 \\
\hline
常数项 & 0.0561 & 0.00294 & 19.09710 & 0.00001 \\
\hline
标普500收益率 & 1.2054 & 0.00298 & 404.25225 & 0.00002 \\
\hline
\end{tabular}
\end{center}

该分析师想要在5\%的显著性水平下检验投资组合的贝塔值等于1.2的零假设。根据回归结果，该分析师将：\\
\end{question}
