...
\begin{question}
    ...
    一位金融研究公司的初级分析师正在研究时间序列分析中的白噪声概念。以下关于白噪声的陈述中，哪一项是错误的？
    \end{question}
...
\begin{question}
    ...
    一位对冲基金的风险分析师正在研究时间序列模型。白噪声是任何时间序列模型的基本构建块。以下关于白噪声过程的性质的陈述中，哪一项是正确的？
    \end{question}
    ...
    \begin{choices}
        ...
        任何白噪声过程都是协方差平稳的，因为前两个矩是时间不变的，且方差有限。
    \end{choices}
...
\begin{question}
    ...
    一位投资银行的风险分析师正在对债券收益率进行时间序列分析。分析师需要确定时间序列是否是协方差平稳的。以下哪一项陈述代表时间序列成为协方差平稳的一个要求？
    \end{question}
    ...
    \begin{choices}
        ...
        当自协方差函数关于位移\(\tau\)不对称时，可以实现前瞻性平稳性。
        ...
        时间序列的分布应该具有接近0的偏度值，使得均值落在分布的中心。
    \end{choices}
...
\begin{question}
    ...
    一位金融机构的风险分析师正在比较移动平均（MA）表示和自回归（AR）过程。以下哪一项陈述是它们之间的关键区别？
    \end{question}
...
\begin{question}
    ...
    一位领先投资公司的风险分析师，Alex Thompson，正在研究科技公司的周期性股票回报的建模。考虑使用AR(1)和MA(1)模型，他在构建这些模型之前做出了以下关于这些模型特征的陈述。哪一项陈述最有可能不正确？
    \end{question}
    ...
    \begin{choices}
        ...
        AR(1)模型是 \(y_t=\delta+\varphi\times y_{t - 1}+\epsilon_t\)，如果\(\varphi\)的绝对值小于1，AR(1)模型是协方差平稳的。
        ...
        MA(1)模型是 \(y_t=\mu+\epsilon_t+\theta\times\epsilon_{t - 1}\)，无论\(\theta\)取何值，MA(1)过程总是协方差平稳的。
        ...
        AR(1)过程的自相关函数随着滞后长度增加而衰减，偏自相关函数随着滞后长度增加而衰减。
        ...
        MA(1)过程的自相关函数随着滞后长度增加而截断，偏自相关函数随着滞后长度增加而衰减。
    \end{choices}
...
\begin{question}
    ...
    一位固定收益投资公司的风险分析师正在分析一个协方差平稳的ARMA(1, 1)模型。模型为 \(Y_t = 0.5 + 0.4Y_{t - 1}-0.7\epsilon_{t - 1}+\epsilon_t\)，其中\(\epsilon_t\sim WN(0,\sigma^2)\)。长期均值 \(E[Y_t]\) 是多少？
    \end{question}
...
\begin{question}
    ...
    \begin{figure}[h]
        ...
        \end{tikzpicture}
    \end{figure}
        一位资产管理公司的风险分析师正在尝试分析和预测特定股票的绩效。分析师收集了该股票的历史时间序列数据。根据下面显示的偏自相关函数（PACF）图，哪一种回归方法最适合这种股票？
        \begin{figure}[H]
            \centering
            \begin{tikzpicture}
                \begin{axis}[
    ...
    \begin{question}
        ...
        一位金融机构的风险分析师正在研究一个协方差平稳的ARMA(2, 2)模型。模型定义为 \(Y_t = 0.3+0.5Y_{t - 1}-0.2Y_{t - 2}+0.7\epsilon_{t - 1}+0.2\epsilon_{t - 2}+\epsilon_t\)，其中\(\epsilon_t\sim WN(0,\sigma^2)\)。长期无条件均值 \(E[Y_t]\) 是多少？
        \end{question}
...
\begin{question}
    ...
    \begin{choices}
        ...
        协方差平稳的时间序列意味着在时间上存在恒定的关系，允许使用历史数据来估计适用于未来样本外观测的模型。
    \end{choices}
...
\begin{question}
    ...
    一位投资公司的风险分析师正在分析一个由三十六季度观测值组成的时间序列（\(m = \text{自由度}=36\)）。分析师计算得到的Box - Pierce Q统计量为28.80，Ljung - Box Q统计量为38.20。分析师想要确定该序列是否为白噪声。已知\(\text{CHISQ.INV}(0.95, 36)=50.99\)，哪一项是最合适的结论？
    \end{question}
    
    \begin{choices}
        \item With 95\% confidence, you accept the series as white noise (more accurately, you fail to reject the null).\\
        在95\%的置信水平下，你接受该序列作为白噪声（更准确地说，你未能拒绝原假设）。
        ...
        在95\%的置信水平下，你拒绝两个原假设但得出该序列是白噪声，因为统计量的和大于临界值。
    \end{choices}
    ...
    \begin{question}
        ...
        一位金融研究公司的风险分析师正在对ARMA模型进行规格分析，通过检验残差中的自相关性。分析师考虑了两个密切相关的检验：Box - Pierce检验和Ljung - Box检验。哪一项陈述准确区分了这两种检验？
        \end{question}
        ...
        \begin{choices}
            ...
            渐近情况下，Ljung - Box检验统计量服从正态分布，而Box - Pierce检验统计量服从卡方分布。
        \end{choices}
...
\begin{question}
    ...
    \begin{choices}
        ...
        Ljung-Box Q统计量用于检验时间序列是否存在线性趋势，在单位根原假设下进行检验。
        ...
        在时间序列的自相关系数联合为零的原假设下，Box-Pierce Q统计量近似服从卡方分布。
        ...
        Ljung-Box检验中，滞后阶数的选择是在联合检验和分布近似质量之间进行平衡。
    \end{choices}
...
在金融风险管理（FRM）中，自相关函数（ACF）和偏自相关函数（PACF）是分析时间序列数据的重要工具。基于提供的材料，哪一项陈述是错误的？
\end{question}
...
\begin{question}
    ...
    一位零售咨询公司的分析师Lily正在建立一个模型来预测客户销售随时间的变化。她已经识别出销售数据中的季度季节性模式。如果Lily在她的模型中包含截距项，她应该使用多少个虚拟变量来建模季节性成分？
    \end{question}
    ...
    \begin{question}
        ...
        \\一位消费品公司的金融分析师正在使用包含虚拟变量的回归模型来分析季度销售额。回归方程为 \(SALES_t = \beta_0+\beta_2D_{2,t}+\beta_3D_{3,t}+\beta_4D_{4,t}+\epsilon_t\)，其中 \(SALES_t\) 是季度销售额的观测值，当 \(t\) 为第二季度时 \(D_{2,t} = 1\)，否则 \(D_{2,t} = 0\)，当 \(t\) 为第三季度时 \(D_{3,t} = 1\)，否则 \(D_{3,t} = 0\)，当 \(t\) 为第四季度时 \(D_{4,t} = 1\)，否则 \(D_{4,t} = 0\)。截距项 \(\beta_0\) 代表第：
        \end{question}
...
\begin{question}
    ...
    \end{tabular}
    \end{center}
    一位投资公司的金融分析师正在检查实际GDP增长率单位根存在的ADF检验结果。下图显示了ADF检验结果。列$\gamma$报告了ADF检验回归的参数估计和ADF统计量（括号内）。列$\delta_0$和$\delta_1$报告了常数和线性趋势的估计以及t统计量。最后一列报告了5\%的临界值。哪一项陈述是正确的？
    \begin{center}
        \begin{tabular}{|c|c|c|c|c|}
        \hline 
        确定性项 & $\gamma$ & $\delta_0$ & $\delta_1$ & 5\%临界值 \\ 
        \hline 
        趋势 & -0.856 (-9.234) & 9.12e-03 (6.321) & -1.65e-05 (-2.567) & -3.512 \\ 
        \hline 
        \end{tabular}
        \end{center}
    \end{question}
    ...
    \begin{choices}
        ...
        如果滞后值的系数统计显著地等于零，ADF检验将拒绝单位根过程的原假设。
        ...
        实际GDP增长率是协方差平稳的，因为$\gamma$统计显著地等于零。
    \end{choices}
...
一位世界银行的研究分析师正在研究全球经济增长模式。分析师观察到世界经济的增长率可以近似为每年固定的百分比增长。哪一种时间序列趋势模型最适合估计GDP？
\end{question}

\begin{choices}
    \item A quadratic trend model: \(GDP_t = \delta_0 + \delta_1t + \delta_2t^2\)\\
    二次趋势模型：\(GDP_t = \delta_0 + \delta_1t + \delta_2t^2\)
    \item A log-log model: \(\ln GDP_t = \ln(\delta_0) + \delta_1\ln t\)\\
    对数-对数模型：\(\ln GDP_t = \ln(\delta_0) + \delta_1\ln t\)
    \item A linear trend model: \(GDP_t = \delta_0 + \delta_1t\)\\
    线性趋势模型：\(GDP_t = \delta_0 + \delta_1t\)
    \item A log-linear model: \(\ln GDP_t = \delta_0 + \delta_1t\)\\
    对数线性模型：\(\ln GDP_t = \delta_0 + \delta_1t\)
\end{choices}
...
一位造纸公司的数据分析师正在建立一个回归模型来分析每月销售的影响。模型需要考虑全年12个月的所有因素。哪一种模型规格会导致完全多重共线性？
\end{question}
...
一位全球投资银行的量化分析师正在开发时间序列模型来预测市场走势。在检查各种金融时间序列时，分析师需要正确识别和处理单位根，以确保模型准确性。基于分析师的研究材料，哪一项关于时间序列分析的陈述是不正确的？
\end{question}
...
\begin{choices}
    ...
    包含单位根的时间序列可能出现伪回归现象，即看似有意义的回归关系实际上可能无法解释经济现象。
    ...
    ARMA模型中，时间序列的平稳性可以通过Dickey-Fuller分布进行检验，该分布的非对称性确实给模型选择带来了困难。
\end{choices}
...
一位摩根士丹利的衍生品交易员正在开发一个均值回归交易策略用于利率产品。在审查各种利率序列的统计特性时，她需要理解单位根对她交易策略的影响。基于她的研究材料，哪一项关于单位根过程的陈述是不正确的？
\end{question}

\begin{choices}
    \item Unit roots has no different with covariance-stationary processes in terms of mean-reversion properties\\
    单位根与协方差平稳过程在均值回归特性方面没有区别
    ...
    利率模型中包含单位根的参数估计服从被称为Dickey-Fuller（DF）分布
    ...
    当利率序列包含单位根时，它可能与具有时间趋势或单位根的其他利率显示虚假关系
...
\begin{choices}
    ...
    基于小样本，检验时间序列的自相关系数联合等于零的假设，分析师应该最适当地计算Box-Pierce（BP）Q统计量
\end{choices}
...
一位纽约联邦储备银行的高级计量经济学家正在分析各种利率序列的平稳性。计量经济学家应用Augmented Dickey-Fuller（ADF）检验来确定特定的利率序列是否为随机游走。如果时间序列是协方差平稳的，这个结果在ADF检验中如何体现？
\end{question}
...
一位全球投资银行的风险分析师正在分析2020年5月16日COVID-19疫情期间的市场数据。在那一天，S\&P 500指数经历了一次陡峭的下跌，触发了一系列自动交易暂停，旨在遏制恐慌性抛售。指数从前一收盘价2,711.02下跌到2,386.13。计算这个市场运动的简单日收益率和日对数收益率。
\end{question}

\begin{choices}
    \item Simple return: -13.62\%, Log return: -12.77\%\\
    简单收益率：-13.62\%，对数收益率：-12.77\%
    \item Simple return: 13.62\%, Log return: 11.65\%\\
    简单收益率：13.62\%，对数收益率：11.65\%
    \item Simple return: 11.98\%, Log return: 12.77\%\\
    简单收益率：11.98\%，对数收益率：12.77\%
    \item Simple return: -11.98\%, Log return: -12.77\%\\
    简单收益率：-11.98\%，对数收益率：-12.77\%
\end{choices}
...
\begin{question}
    ...
    \end{tabular}
    \end{center}
   一位量化投资公司的风险分析师收集了四个不同分布的数据。分析师希望确定在95\%置信水平下，哪些数据集可能来自正态分布，其中卡方临界值为5.99。
   \begin{center}
    \begin{tabular}{|c|c|c|c|c|}
    \hline
    数据集 & 偏度 & 峰度 & T & JB \\
...
\begin{question}
    ...
    一位零售银行的风险分析师正在分析其分支机构的访客数量。访客数量遵循幂律分布，其中\(Prob(v > X) = k \times X^{-\alpha}\)，\(\alpha = 2.0\)。假设4.0\%的分支机构每天有400或更多访客。每天有多少百分比的分支机构有1600或更多访客？
    \end{question}
    ...
    \begin{question}
        ...
        \\一位金融公司的风险分析师正在分析两只股票（股票X和股票Y）收益率之间的关系。该分析师收集了这两只股票在2015年至2020年六年期间的历史收益率，如下表所示：
        \begin{center}
        \begin{tabular}{|c|c|c|}
        \hline
        年份 & X & Y \\
...
\begin{question}
    ...
    \\  一位投资公司的风险分析师正在尝试评估两只股票（股票X和股票Y）收益率之间的关系。该分析师收集了这两只股票在2015年至2020年六年期间的历史收益率，如下表所示：
    \begin{center}
    \begin{tabular}{|c|c|c|}
    \hline
    年份 & X & Y \\
...
\begin{question}
    ...
    \\一位信用评级公司的风险分析师有两家不同的评级机构，Agency A和Agency B，对40家公司进行了信用评级。评级分为五个类别：
    \begin{itemize}
        \item 
        评级1：超级高投资级
        \item 
        ...
        评级5：非投资级
    \end{itemize}
    分析师想要理解两家评级机构给出的评级类别之间的关系。哪一种统计量最合适来近似两家评级机构评级类别之间的联系？
    \end{question}
...
\begin{question}
    ...
    \begin{choices}
        ...
        皮尔逊相关系数用于衡量线性依赖，而斯皮尔曼秩相关系数和肯德尔τ用于衡量单调关系。
        ...
        皮尔逊相关系数和秩相关系数之间的显著差异表明变量之间存在强线性关系。
    \end{choices}
...
\begin{question}
    ...
    一位固定收益投资公司的风险分析师正在对债券投资组合进行风险评估，特别关注尾部风险。在评估高收益企业债券和长期美国国债之间的依赖关系时，分析师对测量它们的非线性依赖感兴趣。分析师考虑使用皮尔逊相关系数、斯皮尔曼相关系数或肯德尔τ来进行此目的。哪一项关于这些度量的陈述最可能正确？
    \end{question}
...
\begin{question}
    ...
    一位对冲基金的风险分析师正在参与一个股票投资组合优化项目。在彭博终端的情景分析中，分析师发现股票投资组合的收益率与天然气价格之间存在强相关性。鉴于投资组合收益率和天然气价格之间的关系既存在线性依赖又存在非线性依赖，分析师旨在考虑这两种依赖关系。哪一项关于测量依赖关系的陈述最不可能正确？
    \end{question}
    ...
    \begin{choices}
        ...
        皮尔逊相关系数和回归斜率之间存在密切的关系，回归斜率只有在相关系数不为零时才不为零。
    ...
    \begin{question}
        ...
        一位全球投资银行的风险分析师正在分析加密货币资产的每日收益率分布。分析师注意到极端价格波动比在正态分布下预期的更频繁，表明存在幂律行为。哪一项关于幂律尾部陈述最准确？
        \end{question}
