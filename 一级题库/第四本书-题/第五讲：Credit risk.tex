...
\begin{question}
    ...
    \end{array}
    \]
    \\一位信用风险分析师正在分析一家大型投资银行的贷款组合的一年评级转移概率。分析师已经编制了以下转移矩阵，并需要验证团队计算的几个概率。以下哪个陈述正确解释了矩阵中显示的转移概率？
    \[
        \begin{array}{|c|c|c|c|c|c|c|c|c|}
        \hline
        \text{从/到（\%）} & \text{AAA} & \text{AA} & \text{A} & \text{BBB} & \text{BB} & \text{B} & \text{CCC/C} & \text{D} \\
        \hline
        \text{AAA} & 87.44 & 7.37 & 0.46 & 0.09 & 0.06 & 0.00 & 0.00 & 4.59 \\
        \hline
        \text{AA} & 0.60 & 86.65 & 7.78 & 0.58 & 0.06 & 0.11 & 0.02 & 0.01 \\
...
\begin{question}
    ...
    \\一位养老基金的投资指南要求立即清算评级低于投资级的债券。在穆迪评级体系下，以下哪个评级变动会触发这一要求？
\end{question}
...
\begin{question}
    ...
    \\一位评级机构的信用风险分析师正在分析公司债券的信用评级稳定性。以下转移矩阵显示了2023年期间在不同评级类别之间迁移的评级发行人数量：

    \begin{center}
    \begin{tabular}{|c|c|c|c|c|c|c|}
    \hline
    \multirow{2}{*}{起始} & \multicolumn{6}{c|}{2023年末} \\
    \cline{2-7}
    & AAA & AA & A & BBB & 违约 & 总计 \\
    \hline
    AAA & 48 & 3 & 2 & 0 & 0 & 53 \\
    ...
    分析师需要计算'A'级发行人的降级概率。在2023年初被评为'A'级的发行人在年底被降级的概率是多少？
...
\begin{question}
    ...
    \\Sarah是绿色能源基金的组合经理，持有NextGen Solar Corp.发行的绿色债券。穆迪将债券评级从A-降至BBB+后，价格下跌了12\%。分析师现在建议潜在的评级上调。关于评级和市场行为，以下哪个陈述是正确的？
\end{question}
    
\begin{choices}
    \item The bond price will fully recover its 12\% loss when the rating returns to A-.\\
    债券价格将在评级恢复到A-时完全收回12\%的损失。
    ...
    CDS利差主要反映历史市场数据，通常滞后于评级变化，提供市场信号。
    ...
    评级机构在所有类型的债券中保持传统财务指标和ESG因素之间的一致权重。
\end{choices}
...
\end{explanation}
    ...
    \begin{question}
        ...
        \\一位评级机构的信用风险分析师正在讨论评级机构通常的做法。分析师提出了几个关于这些做法的陈述。以下哪个陈述是不正确的？
    \end{question}
    ...
    \begin{choices}
        ...
        信用评级机构在债券首次发行时提供初始评级，并定期进行回顾和调整。
        ...
        评级过程考虑定性因素，如公司治理结构和目标公司管理层的访谈结果。
    \end{choices}
...
\begin{question}
    ...
    \\一位评级机构的信用风险分析师正在解释评级展望和评级观察名单的概念。分析师提出了几个关于这些概念的陈述。以下哪个陈述是正确的？
\end{question}
...
\begin{choices}
    ...
    评级展望作为信用质量的实时指标，反映即时市场变化和被评级实体短期财务状况波动。
...
\begin{question}
    ...
    \\一家大型国际银行正在审查其内部信用评级系统。风险管理团队正在讨论是否对不同类型的借款人采用不同的评级方法。在讨论中，提出了几个关于时点（PIT）和周期（TTC）评级方法的陈述。以下哪个陈述是正确的？
\end{question}
...
\begin{choices}
    ...
    时点评级关注长期经济趋势和结构性因素，在不同市场条件下提供稳定的信用评估。
    ...
    周期评级强调实时市场指标和当前财务指标，实现对市场变化的快速响应。
    ...
    时点方法下的信用评分通常比周期评级表现出更高的波动性，因为它们对当前经济状况更敏感。
    ...
    有效的内部评级系统应采用时点方法对中小企业进行评级，同时对大型企业采用周期评级，以优化风险评估。
\end{choices}
...
\begin{question}
    ...
    \\在一次对美国银行量化风险分析团队的信用风险分析师职位的面试中，候选人被问及银行内部评级和外部评级之间的差异。以下哪个陈述是不正确的？
\end{question}
...
\begin{choices}
    ...
    内部评级系统需要多个输入因素和定期回测来保持其有效性。验证过程应检查评级方法和实际表现。
    ...
    Moody's的Baa3评级和S\&P的BBB+评级表明两家主要评级机构对类似违约概率的评估，因为它们使用可比的评级尺度。
    ...
    时点（PIT）和周期（TTC）方法在内部评级系统中各有不同的目的。选择取决于具体应用和业务目标。
    ...
    外部评级机构如穆迪和标准普尔使用标准化方法，而内部评级系统可以根据银行特定风险因素和投资组合特征进行定制。
\end{choices}
...
\begin{question}
    ...
    \\一位固定收益投资公司的风险分析师正在评估贯穿周期（TTC）和在点（PIT）方法在信用评级中的差异。分析师意识到理解这些方法论对于做出明智的投资决策至关重要。在TTC方法和PIT方法之间，以下哪个陈述是正确的？
\end{question}
...
\begin{choices}
    ...
    外部信用评级动态反映市场条件，周期评级迅速调整以捕捉短期市场波动。
    ...
    PIT方法关注长期稳定的信用评估视角，而TTC方法关注更即时和条件性的信用评估视角。
    ...
    在危机时期，PIT方法意味着更高的预期和意外损失（EL \& UL），因此PIT往往具有顺周期性。
\end{choices}
...
\end{explanation}
    ...
    \checklist{对贯穿周期（TTC）和在点（PIT）方法的理解。（如何根据不同方法的定义判断哪种方法更合适。）}
      ...
      \begin{question}
        ...
        \\一位评级机构的信用风险分析师正在讨论信用评级的一致性和影响因素。分析师提出了几个关于这些方面的陈述。以下哪个陈述是正确的？
    \end{question}
    ...
    \begin{choices}
        ...
        评级机构维护行业特定评级模式的全面数据库，使分析师能够得出关于不同行业评级一致性的可靠结论。
        ...
        不同评级机构对银行的评级一致性低于其他公司，这可能是由于银行业务的复杂性较高或评级机构采用不同的评估标准。
        ...
        评级机构追求一致性，从过去数据推断未来是安全的，因为评级机构相信历史数据可以准确预测未来信用风险。
        ...
        评级机构采用标准化的方法，这些方法随着时间的推移保持稳定，因为已建立的评级框架在各种市场条件下都被证明是有效的。
    \end{choices}
...
\begin{question}
    ...
    \\一位评级机构的信用风险分析师正在研究信用评级变动对市场反应的影响。基于历史数据和实证证据，以下哪个市场反应与典型观察模式不一致？
\end{question}
\begin{choices}
    ...
    \\当公司的信用评级从BBB下调至BBB-时，导致其股票和债券价格下跌，同时信用违约掉期（CDS）利差增加。这表明评级下调对市场有显著影响。
    ...
    \\当公司的信用评级从A上调至AA-时，市场反应较为平淡，股票和债券价格变化不大。这表明市场对评级上调的反应不显著。
    ...
    \\当公司的信用评级从AAA下调至AA+时，尽管仍保持在投资级别，但市场可能会强烈反应，导致股票和债券价格大幅下跌。这表明即使是小幅度的评级下调，也可能导致市场的显著波动，尤其是在高评级的情况下，市场对任何负面信息都非常敏感。
    ...
    \\当公司的信用评级从BBB上调至BBB+时，市场反应积极，股票和债券价格上涨，同时信用违约掉期（CDS）利差下降。这表明市场对评级上调的反应非常显著。
\end{choices}
...
\begin{question}
    ...
    \\一位信用风险分析师正在评估不同评级方法论对经济周期的影响。分析师提出了几个关于这些方面的陈述。以下哪个陈述是正确的？
\end{question}
...
\begin{choices}
    ...
    时点评级法通过在整个经济周期中提供一致的评级，增强市场稳定性，从而实现平衡的贷款实践。
    ...
    周期评级法动态响应市场条件，允许银行根据短期经济指标调整其贷款策略。
    ...
    在经济衰退期间，企业需要资金补充流动性，因此银行通常会给出较高评级以支持经济复苏。
    ...
    时点评级法在经济衰退时可能导致贷款规模缩减，从而减缓经济复苏的速度。
\end{choices}
...
\begin{question}
    ...
    \\一位资产管理公司的初级风险分析师正在评估开发公司多样投资组合的内部信用评级的必要性。在团队会议上，分析师提出了几个关于实施这些评级的理由，并询问以下哪个陈述是正确的？
\end{question}

\begin{choices}
    \item One reason for developing internal ratings is that there may not be external ratings available.\\
    开发内部评级的一个原因是可能没有可用的外部评级。
    ...
    会计准则建议银行主要使用市场指标进行贷款估值，违约概率作为补充参考。
...
\begin{question}
    ...
    \end{itemize}
    一家银行正在使用机器学习来自动化其贷款决策过程。为了评估公司的违约概率和信用风险，银行采用了Altman Z-score模型。该模型使用以下公式计算公开交易制造公司的违约概率：
    \[
    Z = 1.2X_1 + 1.4X_2 + 3.3X_3 + 0.6X_4 + 0.999X_5
    \]
    ...
    给定公司的财务比率如下：
    \begin{itemize}
        \item 流动资产与总资产之比 \(X_1 = 0.15\)
        \item 留存收益与总资产之比 \(X_2 = 0.05\)
        \item 息税前利润与总资产之比 \(X_3 = 0.08\)
        \item 股票市值与总负债之比 \(X_4 = 1.2\)
        \item 销售收入与总资产之比 \(X_5 = 0.8\)
    \end{itemize}
...
\begin{question}
    ...
    \\一位投资公司的风险分析师正在举办新员工研讨会，介绍意外损失（UL）的概念。为了演示UL的计算，分析师提供了以下关于假设债券投资组合的数据：
    \begin{itemize}
        \item 债券投资组合的 Principal amount: SGD 150 million  
        \item 投资组合的违约率: 3.0\%  
        \item 回收率: 40\%  
        \item 一年99\% VaR: SGD 10.5 million  
        \item 一年99\% ES: SGD 15.2 million  
    \end{itemize}
...
\begin{question}
   ...
   Default Rate as a function of F:
    ...
    \\一位银行监管机构的初级分析师正在参加内部培训课程，学习Vasicek模型。分析师审查了与模型相关的以下方程：
    \[
    U_i = a_iF + \sqrt{1 - a_i^2}Z_i
    \]
...
\begin{choices}
    ...
    投资组合中每个贷款的违约概率分别映射到标准正态分布 \(U_i\)，其中极右尾的值代表违约。
    ...
    模型系数 \(a\) 直接关系到投资组合中贷款的违约概率分布 \(U_i\) 之间的相关性。
\end{choices}
...
\begin{question}
    ...
    \\一位投资者持有价值150百万美元的投资组合。该投资组合由A评级债券（60百万美元）和BBB评级债券（90百万美元）组成。假设A评级债券和BBB评级债券的一年违约概率分别为4\%和6\%，并且它们是独立的。如果A评级债券在违约事件中的回收价值为75\%，而BBB评级债券的回收价值为50\%，那么该投资组合的一年预期信用损失是多少？
    \end{question}
...
\begin{array}{|c|c|c|c|}
\hline
\text{贷款} & \begin{tabular}{c}\text{违约时} \\ \text{敞口（百万）}\end{tabular} & \begin{tabular}{c}\text{一年期违约} \\ \text{概率（\%）}\end{tabular} & \begin{tabular}{c}\text{违约} \\ \text{损失率（\%）}\end{tabular} \\
\hline
a & 50.00 & 5.00 & 80.0 \\
\hline
b & 70.00 & 2.00 & 50.0 \\
...
风险率（也称为违约强度）在定义上是连续的，但假设为离散也是可以的，因为这里的差异并不重要。哪笔贷款的预期损失（以美元计）最高？
    \begin{choices}
        \item Loan (a)  
        \item Loan (b)  
        \item Loan (c)  
    ...
    \begin{question}
        ...
        \\一位金融咨询公司的风险分析师正在审查信用风险管理中的预期损失（EL）的组成部分。分析师理解EL对于评估贷款组合的潜在损失至关重要。基于这个认识，分析师考虑了EL的三个组成部分：违约概率（PD）、暴露金额（EA）和损失率（LR）。以下哪个陈述是不正确的？
    \end{question}
    ...
    \begin{choices}
        ...
        暴露金额（EA）是信用损失的标准差，在排除未付利息的情况下，在期限结束时估计。
        ...
        违约概率（PD）是借款人在预定时间段结束之前违约的概率（通常选择的估计期限为一年）。
        ...
        损失率（LR）表示在违约时实际损失与暴露金额的比率（包括与收回和出售抵押品相关的所有成本）。
        ...
        \]
        \\预期损失（EL）等于以下内容的乘积：到时间H（期限）时的违约概率；时间H时的暴露金额；以及时间H时的损失率；即，
        \[
        EL(H) = PD(H) \times EA(H) \times LR(H).
        \]
    \end{choices}
...
\\一位银行的风险经理正在向一组分析师讲解如何估计贷款组合中的信用损失。该经理提出了一个由两笔贷款组成的投资组合场景，并提供如下贷款信息：
...
\begin{array}{|c|c|c|}
\hline
\text{贷款} & \text{贷款1} & \text{贷款2} \\
\hline
\text{借款金额} & \text{1500万元人民币} & \text{2000万元人民币} \\
\hline
\text{违约概率} & 2\% & 2\% \\
\hline
\text{回收率} & 40\% & 25\% \\
\hline
\text{贷款1和贷款2之间的违约相关性} & 0.6 & - \\
\hline
\end{array}
\]
}

假设投资组合损失服从二项分布，投资组合损失的标准差估计值是多少？
...
\begin{question}
    ...
    \\一位银行的风险分析师正在评估刚刚获批的三年期贷款的年违约概率。分析师从评级机构数据中确定，从今天到年底3年的平均风险率为3\%，并使用这一信息计算借款人的一年生存率。如果年末1年和年末3年之间的无条件违约概率为1.2345\%，那么借款人在贷款第一年的生存率是多少？
\end{question}
...
\begin{question}
    ...
    \\一家银行有价值100百万美元的贷款组合，违约概率为0.75\%。相关参数估计为0.2，违约事件的回收率为30\%。假设Vasicek模型给出的99.9\%违约率是12.01\%，那么所需的监管资本是多少？
\end{question}
...
\begin{question}
    ...
    \\一个投资组合包含三个独立的债券，每个债券的面值为100美元，违约概率为3\%，违约损失率为100\%。分别计算95\%和99\%置信水平下的投资组合价值VaR。
    \end{question}
    ...
    \begin{question}
        ...
        \\一位风险管理顾问正在向一家申请银行牌照的金融科技公司提供建议。该公司需要了解监管资本和经济资本在风险管理框架中的差异。在比较这两种资本要求时，以下哪个陈述是正确的？
     \end{question}
        ...
        \begin{choices}
            ...
            银行监管机构要求银行保持监管资本，并在资本计算中考虑风险之间的相关性。
            ...
            使用CreditMetrics模型进行经济资本计算倾向于比Basel II监管资本更为保守，当目标违约概率为0.01\%时。
            ...
            银行的经济资本可以通过简单地将信用风险、市场风险和操作风险的资本要求相加来计算。
            ...
            监管资本和经济资本主要用于覆盖预期损失，而保险产品用于覆盖意外损失。
        \end{choices}
        ...
        \begin{question}
            ...
            \\一位量化研究员Sarah正在为Nova Investment的新结构化产品投资组合开发信用风险模型。在利用市场数据校准模型参数时，她遇到了几个方法论上的挑战。以下关于信用风险参数估计的哪个陈述是正确的？
            \end{question}
            ...
            \begin{choices}
                ...
                模型需要通过周期和时点估计回收率，而不是违约概率（PD），以满足监管和会计标准。
                ...
                模型假设在市场压力期间，回收率增加，抵消更高的违约率，从而保持稳定的信用风险水平。
                ...
                模型表明交易对手的违约概率倾向于随着与该交易对手的交易敞口增加而下降。
                ...
                模型需要考虑风险因子相关性，因为不同风险类型相互作用并影响总体资本要求。
            \end{choices}
            ...
            \begin{question}
                ...
                \end{tabular}
                \end{table}
                根据下表中显示的累积违约概率，计算前4年的累积生存率、第5年的无条件违约率以及在前4年未发生违约的情况下第5年的条件违约概率。以下哪个选项是正确的？
                ...
                年份 & 1 & 2 & 3 & 4 & 5 & 6 \\
                \hline
                累积违约概率 & 3.76\% & 8.56\% & 12.66\% & 15.87\% & 18.32\% & 20.32\% \\
                \hline
                \end{tabular}
                \end{table}
            \end{question}
...
\begin{question}
    ...
    \\假设一家银行被评为AA级，AA级公司的违约概率通常为0.02\%。该银行的当前借款人违约概率为1.305\%，回收率为25\%。在损失分布中，99.9\%分位数对应于14.89\%的违约概率，99.98\%分位数对应于22.31\%的违约概率。计算银行的监管资本和经济资本。
\end{question}

\begin{choices}
    \item Regulatory capital: 10.19\%, Economic capital: 15.75\% \\
    监管资本：10.19\%，经济资本：15.75\%
    \item Regulatory capital: 9.50\%, Economic capital: 15.75\% \\
    监管资本：9.50\%，经济资本：15.75\%
    \item Regulatory capital: 10.19\%, Economic capital: 16.00\% \\
    监管资本：10.19\%，经济资本：16.00\%
    \item Regulatory capital: 8.00\%, Economic capital: 15.75\% \\
    监管资本：8.00\%，经济资本：15.75\%
\end{choices}
...
\begin{question}
    ...
    \\一家银行正在审查其资本要求，以确保它符合监管和内部风险管理标准。银行的风险管理团队正在讨论经济资本和监管资本之间的差异。以下关于经济资本和监管资本的哪个陈述是不正确的？
\end{question}

\begin{choices}
    \item Economic capital is calculated by banks based on their own risk conditions to absorb unexpected losses. \\
    经济资本是银行根据自身风险状况计算的资本，用于吸收非预期损失。
    ...
    监管资本是监管机构要求银行持有的最低资本，以确保银行能够抵御风险。
...
\begin{question}
    ...
    Which of the following statements regarding the Vasicek model is correct?
    \\    一位银行监管机构的初级分析师正在参加关于Vasicek模型的内部培训课程。该分析师回顾了与该模型相关的以下方程：
...
\begin{choices}
    ...
    投资组合中每个贷款的违约概率分别映射到标准正态分布 \(U_i\)，其中极右尾的值代表违约。
    \item A low value of the factor \(F\) indicates that the economy is strong, while a high value of \(F\) represents economic weakness.\\
    低值的 \(F\) 表示经济强劲，而高值的 \(F\) 表示经济疲弱。
    
    \item For corporate borrowers, the value of the factor \(F\) is higher for loans to companies with more cyclical businesses.\\
    对于企业借款人，贷款给更多周期性企业的公司的 \(F\) 值更高。
    
    \item The model coefficient \(a\) directly relates to the correlations between the default probability distributions \(U_i\) of the loans in the portfolio.\\
    模型系数 \(a\) 直接与投资组合中贷款的违约概率分布 \(U_i\) 之间的相关性相关。
\end{choices}
...
\begin{question}
    ...
    \\假设有一个由贷款A和贷款B组成的投资组合。贷款A和贷款B的损失标准差分别为2和6，贷款A和贷款B之间的相关系数为0.5，投资组合的标准差为7.2111。计算贷款A和贷款B对投资组合风险的影响。
\end{question}
...
\begin{question}
    ...
    \\一家金融机构的风险经理正在准备一份关于衍生品信用风险测量的报告，供银行高级管理团队的成员参考。在研究过程中，他们识别出需要解决的几个关键挑战。在他们的报告笔记中，他们列出了各种关于这些挑战的陈述。以下哪个陈述准确反映了他们所识别的测量衍生品信用风险复杂性？
\end{question}
...
\begin{choices}
    ...
    银行的风险评估模型主要关注时点估计，因为它们提供了市场周期内信用风险最全面的视角。
    ...
    违约损失率（LGD）与市场条件呈正相关关系，表明在违约率上升期间回收率更高。
    ...
    预期违约损失（EAD）计算复杂，可能包括错误方向风险，这在衍生品交易中尤为明显。
    ...
    交易对手风险通常是单边的，所有与交易对手未结清的衍生品都被视为单个衍生品交易对手违约。
\end{choices}
