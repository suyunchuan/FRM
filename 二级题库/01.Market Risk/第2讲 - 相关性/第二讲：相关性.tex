...
\begin{question}
    ...
    假设一个人购买了一个固定相关性为0.3、名义价值为200万美元的互换合约，期限为一年。三个资产的日对数回报的实现两两相关性在到期时为$\rho_{1,2}= 0.8$，$\rho_{3,1} = 0.3$，$\rho_{3,2} = 0.4$。互换买方的到期支付是多少？
\end{question}
...
\begin{question}
    ...
    \\从由多个连续边际分布函数创建的联合分布函数中提取依赖结构的风险度量被称为：
\end{question}
...
\begin{question}
    ...
    \\假设一个债权人向公司A和公司B分别贷款600万美元。根据该行业公司的历史信息，公司A和公司B的违约概率均为7\%，违约相关系数为0.6。在假设损失给定违约的情况下，最坏情况下债权人的预期损失最接近：
\end{question}
...
\begin{question}
    ...
    \\一个基金经理拥有一个非分红股票XYZ的期权组合。组合由18,000张深度实值看涨期权和45,000张深度虚值看涨期权组成。组合还包含22,000份远期合约。XYZ交易价格为95美元。假设一年有252个交易日，XYZ的年波动率为28\%，以下哪个金额最接近组合在95\%置信水平下的1天VaR？
\end{question}
...
\begin{question}
   ...
   \\    Michael Wang是一位风险分析师，他想使用copula（连接函数）来建模资产收益率之间的依赖关系，并且必须说服他的经理这是最佳方法。以下哪些陈述是正确的？

   I. 投资组合资产收益率分布之间的依赖关系对风险测量至关重要。

   II. 相关性估计在低市场波动期间通常显得稳定，然后在压力市场条件下变得不稳定。因此，使用长期估计的相关性计算的风险度量将在压力期间低估风险。

   III. 皮尔逊相关性是两个资产收益率之间依赖关系的线性度量，等于资产收益率的协方差与其波动率乘积的比值。

   IV. 使用copula，可以从边际分布函数构建联合收益率分布函数，这种方式允许资产收益率具有更一般的依赖结构类型。 
\end{question}
...
\begin{question}
    ...
    \\投资组合（X）和（Y）每个都有30\%的波动率，但投资组合（Y）具有更高的回报，因此其绝对VaR较低；即，绝对VaR = - 回报 * T + 偏差 * 波动率 * $\sqrt{T}$。哪种一致性属性说明了这一点？
\end{question}
...
\begin{question}
    ...
    \\一个投资银行分析师正在计算一个相关性互换的收益，该互换即将到期。相关性互换的名义金额涵盖了四个等权重股票。分析师使用以下实现的成对相关性来计算将应用于确定互换支付的相关性：
    \begin{table}[H]
        \centering
        \renewcommand{\arraystretch}{1.2}
        \begin{tabular}{|l|c|c|c|c|}
            \hline
            & \textbf{股票A} & \textbf{股票B} & \textbf{股票C} & \textbf{股票D} \\
            \hline
            \textbf{股票A} & 1.0 & 0.5 & 0.7 & 0.6 \\
            \textbf{股票B} & 0.5 & 1.0 & 0.3 & 0.4 \\
            \textbf{股票C} & 0.7 &0.3 & 1.0 & 0.8 \\
            \textbf{股票D} & 0.6 & 0.4 & 0.8 & 1.0 \\
            \hline
        \end{tabular}
        \caption{股票相关系数矩阵}
    \end{table}
...
\begin{question}
    ...
    \\Beta Capital Inc. (Beta)是一家财富管理公司，目前管理着一个另类投资基金、一个成长型股权基金和一个包括资产Z的个体资产。这两个基金都与同一个定制指数进行基准比较。另类投资基金在MBS中对优先层和夹层层进行了等额投资，该MBS由Bank Alpha发行，由SPV发行。Bank Alpha在交易期间保留了6\%的净经济利益。为了对冲其头寸，Beta从Bank Gamma购买了MBS的CDS。Bank Alpha和Bank Gamma都依赖于零售存款来为其贷款活动融资。Beta使用RAROC在99\%置信水平下评估其投资业绩和资本预算。CIO预计市场波动性和定制指数中的资产相关性在未来一年会增加。CIO然后决定执行以下两个策略：
    ·进入一个支付波动率互换，其中预先指定的固定波动率是Asset Z的当前波动率。
    ·在市场上建立一个多头头寸，与Beta作为固定相关性支付方。

    假设CIO的预期是正确的，以下哪个陈述是正确的？
\end{question}
...
\begin{choices}
    \item Assuming no low-risk anomaly, Beta would generate a gain from the volatility swap if the volatility of Asset Z decreases.\\
        假设没有低风险异常，如果Asset Z的波动率下降，Beta将从波动率互换中获得收益。
    \item If the realized correlation in the correlation swap is higher than the fixed correlation, Beta would generate a gain on the correlation swap.\\
        如果相关性互换的实现相关性高于固定相关性，Beta将在相关性互换中获得收益。
    \item An increase in the default correlation between Bank Alpha and Bank Gamma, all else held constant, would lead to a decrease in the value of Beta's CDS position.\\
        银行Alpha和银行Gamma之间的违约相关性增加，所有其他条件不变，将导致Beta的CDS头寸价值下降。
    \item Holding all else constant, Beta's estimated after-tax RAROC would not change if it adopts a 95-Confidence level compared to a 99-Confidence level for its capital budgeting.\\
        所有其他条件不变，如果Beta采用95\%置信水平进行资本预算，而不是99\%置信水平，其估计的税后RAROC将不会发生变化。
\end{choices}
...
\begin{question}
    ...
    一个风险经理使用标普500指数过去500个月的月度相关性数据来估计普通股票的长期平均相关性和均值回归率。根据历史数据，标普500指数股票的长期平均相关性为35\%，回归输出估计了以下回归关系：Y = 0.225 – 0.80X。假设在2023年3月，标普500指数所有股票的平均月度相关性为40\%。假设回归分析中估计的均值回归率，2023年4月的预期相关性是多少？
\end{question}
...
\begin{question}
    ...
    \\假设一个风险经理检查了标普500指数股票在1980年至2020年期间的相关性和相关性波动。扩张期定义为美国国内生产总值（GDP）增长率大于3.5\%的时期，正常期定义为GDP增长率在0到3.5\%之间的时期，衰退期定义为两个连续的负GDP增长率时期。以下哪个陈述描述了样本的相关性和相关性波动？风险经理最有可能发现：
\end{question}
...
\begin{question}
    ...
    \\假设一个变量在时间周期t-1的值为35。假设该变量的长期均值为45，忽略大多数金融数据回归中包含的随机项。如果均值回归率为0.5，则下一期的变量值预期变化是多少？
\end{question}
...
\begin{question}
    ...
    \\在估计相关性矩阵时，风险经理通常假设相关性的潜在分布。以下哪个陈述最准确地描述了股权相关性分布、债券相关性分布和违约概率相关性分布的最佳拟合分布？股权、债券和违约概率相关性分布的最佳拟合分布分别是：
\end{question}
...
\begin{question}
    ...
    \\在估计相关性矩阵时，风险经理通常假设相关性的潜在分布。以下哪个陈述最准确地描述了股指相关性分布、商品相关性分布和信用利差相关性分布的最佳拟合分布？股指、商品和信用利差相关性分布的最佳拟合分布分别是：
\end{question}
...
\begin{question}
    ...
    \\以下哪个陈述最准确地描述了Student's t copula？
\end{question}
...
\begin{choices}
    ...
    marginal distributions in order to define the correlation matrix.\\
        一个Student's t copula模型的主要缺点是转换原始边际分布以定义相关矩阵。
...
\begin{question}
    ...
    \\以下哪个陈述关于相关性和联结函数是正确的？

    I. Copula允许我们独立于边际分布建模变量之间的依赖结构。

    II. 变量的线性变换保留了它们的相关结构。

    III. 相关性可以有效地用于测量具有无限方差的变量之间的关系。

    IV. 当变量遵循多元正态分布时，相关性是依赖关系的适当度量。
\end{question}
...
\begin{question}
    ...
    \\一个风险经理想要研究一个依赖于两个经济变量X和Y的组合。X在5和9之间均匀分布，Y在6和10之间均匀分布。风险经理想要建模它们的联合分布H(X,Y)。Sklar定理证明，对于任何联合分布H，都存在一个联结函数C，使得：
\end{question}
...
\begin{choices}
    ...
    density marginal distribution of Y.\\
        H(X,Y)等于C[u,d]，其中u是X的密度边际分布，d是Y的密度边际分布。
    \item H(X,Y) is equal to C[(X – 5)/4, (Y – 6)/4].\\
        H(X,Y)等于C[(X – 5)/4, (Y – 6)/4]。
    \item H[(X - 5)/4, (Y- 6)/4] is equal to C(X,Y).\\
        H[(X - 5)/4, (Y- 6)/4]等于C(X,Y)。
\end{choices}
