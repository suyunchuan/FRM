...
\begin{question}
    ...
    \\ 一个风险敞口的压力违约概率为4.0\%，违约损失率为50.0\%。组合价值为100万美元。LGD的标准差为25.0\%。压力下的预期损失是多少？
\end{question}
...
\begin{question}
    ...
    \\ 以下哪一种模型计算由于基础债券的信用迁移（如信用利差的变化）而导致的投资组合价值的变化？
\end{question}

\begin{choices}
    \item Credit Risk+\\信用风险+模型
    \item Credit Metrics\\信用风险度量
    \item KMV\\KMV模型
    \item Both a and c above are true\\A和C选项都是正确的
\end{choices}
...
\begin{question}
    ...
    \\ Lisa Chen是太平洋银行的新贷款官员。她的经理要求她这个月发放五笔商业贷款。Chen听说了一个有良好声誉的当地企业家。她与企业家David Wang见面，被他的商业头脑所打动。他们讨论了Wang的新项目贷款，Chen同意这是一个有前途的机会。她说服贷款委员会，太平洋银行很幸运能与Wang这样的人合作。这是一个例子：
\end{question}
...
\begin{question}
    ...
    \\ 一家银行与另一家银行有一笔总额为500,000美元的贸易，回收率为70\%。银行估计另一家银行有2\%的概率违约。银行的预期损失是多少？
\end{question}
...
\begin{question}
    ...
    \\ Michael Zhang，City Bank的信用分析师，正在审查一家本地汽车经销商的贷款申请。该经销商由David Lee拥有超过20年，销售美国汽车。由于其郊区位置，该经销商主要销售SUV和卡车。然而，销售下降，汽油价格继续上涨。Lee正在考虑增加混合动力汽车到他的库存。Lee之前从City Bank借款，但目前没有未偿还贷款。以下哪一项陈述不是信用分析Zhang应该评估的四个组成部分之一？
\end{question}
...
\begin{question}
    ...
    \\ 一位分析师正在使用Moody's KMV模型估计大型公共公司Sports Inc.的违约距离。该公司设计、制造和销售运动鞋。该公司的资本结构包括4000万美元的短期债务、2000万美元的长期债务和目前交易的一百万股股票，每股价格为10美元。资产波动率为每年20\%。Sports Inc.的标准化违约距离是多少？
\end{question}
...
\begin{question}
    ...
    \\ David Wang是一位投资银行的高级分析师，准备研究矿业部门的报告。Wang注意到他关注的一家公司最近增加了对公司资产的风险，他认为这将有利于股权持有人，但损害债务持有人的利益。以下哪一个概念最准确地描述了Wang分析中的情况？
\end{question}
...
\begin{question}
    ...
    \\ 以下每一项都代表在评估违约概率时理想情况下应该捕捉到的定性信息，除了哪一项？
\end{question}
...
\begin{question}
    ...
    \\ Golin和Delhaise根据借款人类型将信用分析分为四个领域：
    \begin{enumerate}
        \item 消费者信用分析：评估单个消费者的信用worthiness
        \item 非金融公司信用分析：评估非金融公司
        \item 金融机构信用分析：评估金融机构
        \item 主权/市政信用分析：评估国家政府和政府的信用风险
    \end{enumerate}
    Golin和Delhaise认为，关于借款人类型信用分析的关键特征，以下哪一项陈述是正确的，除了哪一项是错误的？
\end{question}
...
\begin{choices}
    ...
    个人（消费者）：信用分析适合自动化和使用评分模型，因为可以用有限的变量评估风险，可以使用统计工具建立标准化评分模型
    ...
    非金融公司：与消费者相比，倾向于更详细和“手动”（即自动化程度更低）；关键变量可能包括流动性、现金流、短期盈利能力和盈利能力、偿付能力或资本状况
    ...
    金融机构：与非金融公司信用分析相比，定性分析和资产质量不重要，但现金流是一个非常重要的指标（一个“关键指标”）
    ...
    主权：包括国家风险分析（主要是政治动态和经济状况）和系统性风险分析（包括监管制度和金融体系）
\end{choices}
...
\begin{question}
    ...
    \end{itemize}
    以下哪一项陈述关于信用违约互换（CDS）是不正确的？
    \begin{itemize}
        \item I.CDS的购买者确实寻求信用保护，通过购买CDS来对冲参考债务的信用风险。
        \item II.CDS的支付不是基于参考债务的基准信用利差。CDS买方在合约期限内或发生信用事件前，向CDS卖方支付固定费用（spread），这是预先确定的固定金额，通常以基点表示的年化费用，而不是基于基准信用利差。
        \item III.实物结算提供CDS买方参考债务的面值。
        \item IV.CDS具有与分期付款的信用违约看跌期权相同的收益结构，两者都提供信用保护。
    \end{itemize}
\end{question}
...
\begin{question}
    ...
    \\ 一个2年的信用违约互换（CDS），指定实物交割在两年后违约。如果参考资产是\$200 million, 8.0\% ABC公司债券，CDS利差为125个基点，CDS买方将：
\end{question}
...
\begin{question}
    ...
    \\ 信用风险的经济资本计算假设回收率（定义为1-损失率）。回收率取决于基础对手方的业务模式和资产价值和规模的波动性。在正常预期情况下，哪种类型的公司将具有最高的回收率？
\end{question}
...
\begin{question}
    ...
    \\ 一位分析师收集了关于ABC Inc.和DEF Inc.的信息。各自的信用评级为AA和BBB，1年期CDS利差分别为200和300个基点。基于公开报告的违约概率分别为10\%和20\%。以下哪一项陈述关于回收率是最可能正确的？
\end{question}
...
\begin{question}
    ...
    \\ 一个AA评级的发行人的6年期CDS，半年支付一次，利差为150个基点，而该发行人的6年期半年付息债券的收益率是8\%。CDS没有交易对手风险。每年6个月支付的LIBOR利率为4.6\%。哪一种策略可以利用套利机会？你的收益将超过LIBOR多少？
\end{question}
...
\begin{choices}
    ...
    没有套利机会，因为任何明显的无风险收益都是对暴露于发行人信用风险的补偿
\end{choices}
...
\begin{choices}
    ...
    \\ 企业债券的违约历史频率为6\%。使用Altman's Z-score分析更新后，违约概率与6\%略有不同
    ...
    \\ 违约距离为1.96，历史上具有这个特征的公司有1.2\%违约，所以违约概率为1.2\%
\end{choices}
...
\begin{question}
    ...
    \\ 一个面值为\$1,000的债券剩余期限为10年。使用Merton模型，债券的现值为\$750。假设无风险利率为3\%，这个债券的信用利差是多少？
\end{question}
...
\begin{question}
    ...
    \\ ABC Corporation的资本结构由两部分组成，一个是面值为\$120 million的4年期债券，其余是股权。公司资产的现值为\$150 million，预期增长率是20\%，波动率为25\%。公司风险管理部门使用Merton模型估计违约距离。给定违约距离，估计的物理违约概率是多少？（N(1.85) = 96.78\%; N(2.45) = 99.29\%）
\end{question}
...
\begin{choices}
    ...
    实证结果表明，Merton模型比简单模型更能预测投资级债券的违约
\end{choices}
...
\begin{question}
    ...
    \\ 假设一家公司有两项债务到期。一项是优先债务，到期时间为4年，面值为\$120 million。另一项是次级债务，到期时间为4年，面值为\$60 million。年利率为4\%，公司价值的波动率为20\%。如果公司价值的波动率在Merton模型中下降，那么以下哪一项陈述是正确的？
\end{question}
...
\begin{choices}
    ...
    如果公司陷入财务困境（公司价值低），则优先债务的价值将上升，而次级债务和股权的价值都将下降
    ...
    如果公司没有陷入财务困境（公司价值高），则优先债务、次级债务和股权的价值都将上升
    ...
    如果公司陷入财务困境（公司价值低），则优先债务和次级债务的价值将上升，而股权的价值将下降
    ...
    如果公司没有陷入财务困境（公司价值高），则优先债务的价值将上升，而次级债务和股权的价值都将下降
\end{choices}
...
\begin{question}
    ...
    \\ 指数分布的速率参数衡量事件发生的速率。在等待公司违约的情况下，速率参数被称为危险率，表示违约到达的速率。假设违约强度为0.15，以下哪一项关于危险率（即违约强度）的陈述是正确的？
\end{question}

\begin{choices}
    \item The cumulative default probability after 3 periods = $e^{-(0.15)3}$\\
    3个周期后的累计违约概率 = $e^{-(0.15)3}$
    \item The conditional default probability after 1 period = $1 - e^{-(0.15)}$\\
    1个周期后的条件违约概率 = $1 - e^{-(0.15)}$
...
\begin{question}
    ...
    \\ 使用Merton模型，计算一家公司的股权和债务的当前价值，假设公司资产现值为\$120 million，四年后面值为\$100 million的零息债券的现值为\$73.50 million，年利率为8\%，公司资产的波动率为25\%。
\end{question}
...
\begin{question}
    ...
    \\ 一个金融分析师想要估计一家公司的股权价值，使用Merton模型。公司不支付股息，资产的市场价值为8000万欧元。它的唯一债务是一张三年后到期的零息债券，面值为5000万欧元，当前价格为62.50欧元/100欧元面值。附加信息如下：
...
\begin{question}
    ...
    \\ 一个信用分析师正在估计一家银行的信用风险经济资本，置信水平为99.9\%。银行报告其当前信用风险组合具有以下参数：
    \begin{itemize}
    \item 总敞口金额：25000万英镑
    \item 违约概率（PD）：2.5\%
    \item 损失率（LR）：12\%
    \item PD的标准差：8\%
    \item LR的标准差：15\%
    \item 99.9\%置信水平下的资本乘数：7.50
    \end{itemize}
    银行应该为其信用风险组合持有多少经济资本？
\end{question}
...
\end{itemize}
    ...
    \\    一个信用分析师团队正在使用Merton模型估计一家非分红公司的信用评级。该团队收集了该公司的相关信息如下：
    \begin{itemize}
    \item 总资产价值：2.5亿澳元
    \item 长期债务：1.2亿澳元
    \item 短期债务：0澳元
...
\begin{question}
    ...
    and nonfinancial service providers; 
    
    Ⅲ.Financial institution credit analysis is the evaluation
    of financial companies including banks and nonbank financial institutions, such as insurance
    companies and investment funds; 
    
    Ⅳ.Sovereign/municipal credit analysis is the evaluation of the
    credit risk associated with the financial obligations of nations, subnational governments, and public authorities, as well as the impact of such risks on obligations of nonstate entities operating in specific jurisdictions. According to Smith and Johnson, each of the following is true about key features of credit analysis with respect to borrower type, except which is not true?\\
    Smith和Johnson将信用分析分为四个领域，根据借款人类型：

    Ⅰ.消费者信用分析是对个人消费者的信用worthiness的评估；

    Ⅱ.企业信用分析是对非金融公司（如制造商、非金融服务提供商）的信用worthiness的评估；

    Ⅲ.金融机构信用分析是对金融机构（包括银行和非银行金融机构，如保险公司和投资基金）的信用worthiness的评估；

    Ⅳ.主权/地方政府信用分析是对与国家、地方政府和公共机构相关的金融义务的信用风险评估，以及这些风险对特定司法管辖区内非国家实体义务的影响。

    根据Smith和Johnson，以下关于借款人类型信用分析的关键特征的陈述，除了哪一项不是正确的？
\end{question}
...
\begin{choices}
    ...
    个人（消费者）：信用分析可以自动化，可以使用评分模型和统计工具将风险与有限数量的变量相关联。
    ...
    非金融公司：与消费者相比，更详细和“手工”（即自动化程度较低）；关键变量可能包括流动性、现金流、近期的盈利能力和盈利能力、偿债能力或资本状况。
    ...
    indicator")\\
    金融机构：与企业（非金融）信用分析相比，定性分析和资产质量不重要，但现金流是一个非常重要的指标（“关键指标”）。
...
\begin{choices}
    ...
    贷款人通过欺骗手段隐瞒贷款的真实性质，例如将可调整利率贷款伪装成固定利率贷款，这属于掠夺性贷款行为。
\end{choices}
...
\begin{question}
    ...
    \\ 在2007-2009年金融危机之前，投资者和评级机构激励错位的主要原因是什么？
\end{question}
...
\begin{question}
    ...
    \\    年初，一家公司以每100美元面值115美元的价格购买了一只AA级公司债券。使用市场数据，风险管理师基于经济学团队提供的利率模拟估计了该债券的以下年末价值：
...
\begin{question}
    ...
    \\ 一家银行的风险分析师正在准备一份关于气候相关金融风险的报告。分析师专注于气候相关金融风险的放大和缓解机制，并评估它们的功能。分析师在报告中应该包含以下哪一项陈述？
\end{question}
...
\begin{question}
    ...
    \\ 你目前持有\$12,000,000面值，7\% XYZ债券。为了对冲你的头寸，你必须决定是购买5年期CDS，每年支付70个基点，还是购买数字互换，每年支付60个基点，赔付率为55\%。一年后，XYZ违约了，目前交易价格为面值的65\%。以下哪一项陈述是正确的？
\end{question}
...
\begin{choices}
    ...
    \\ 在单一名称CDS下，保护买方支付给保护卖方的或有支付大于数字互换
    ...
    \\ 在单一名称CDS下，保护买方支付给保护卖方的或有支付小于数字互换
    ...
    \\ 在单一名称CDS下，保护卖方支付给保护买方的或有支付大于数字互换
    ...
    \\ 在单一名称CDS下，保护卖方支付给保护买方的或有支付小于数字互换
\end{choices}
...
\begin{choices}
    \item Call option on the reference obligation for the buyer of the swap\\ 互换买方对参考债务的看涨期权
    \item Put option on the reference obligation for the buyer of the swap\\ 互换买方对参考债务的看跌期权
    \item Look back option on the reference obligation for the buyer of the swap\\ 互换买方对参考债务的回望期权
    \item Up-and-out option on the reference obligation for the buyer of the swap\\ 互换买方对参考债务的向上敲出期权
\end{choices}
...
\begin{choices}
    \item Has no risk exposure to the combined holdings of the CDS and reference obligation
    \\ CDS买方没有对CDS和参考债务的组合头寸的风险敞口
    \item Is subject to the credit risk of the seller
    \\ CDS买方面临卖方的信用风险
    \item Must place collateral with the seller of the CDS to ensure performance
    \\ CDS买方必须向CDS卖方提供抵押品以确保履行
    \item Can surrender the CDS for the value of the accumulated payments
    \\ CDS买方可以放弃CDS，获得累积付款的价值
\end{choices}
...
\begin{question}
    ...
    \\ 一个量化风险分析师正在研究金融资产之间的相关性，使用过去二十年的历史数据，特别关注2007-2009年全球金融危机。分析师还检查了用于获得对相关性敞口或对冲相关性变化的工具和产品。分析师可以正确得出以下哪一项结论？
\end{question}
...
\begin{choices}
    ...
    \\ CDO分层的价格利差将在系统性房地产市场崩溃期间随着抵押贷款违约概率的增加而减小
    ...
    \\ VaR和ES模型都考虑了资产之间动态金融相关性的影响，以管理风险和估计资本要求
    ...
    \\ 相关性互换是市场上唯一可用的金融产品，用于对冲或扩大资产之间相关性的敞口
    ...
    \\ 如果参考资产与CDS卖方之间的违约相关性增加，CDS利差会下降
\end{choices}
