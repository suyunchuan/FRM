...
\begin{choices}
    ...
    发起人通常比安排人对贷款质量有信息优势，这可能促使发起人与借款人合谋伪造贷款申请
...
\begin{question}
    ...
    \\ 假设有100笔相同的贷款，每笔贷款本金为\$500,000。根据信用分析，对借款人应用300个基点的利差。LIBOR目前为4\%，年息票率将重新设定。高级、次级和股权层分别为75\%、20\%和5\%的池。高级和夹层层的利差分别为2\%和6\%。超过\$1,000,000的剩余现金流被转移到信托账户。假设违约率为2\%。第一期夹层和信托账户的现金流是多少？
\end{question}
...
\begin{question}
    ...
    \\ 假设一家公司只发行三种资本票据：面值为\$300百万的零息高级债；面值为\$500百万的零息次级债；以及\$200百万的股权。高级债的信用增级是多少？
\end{question}
...
\begin{question}
    ...
    \\ 一个高收益债券池被放置在一个SPV中，并发行了三个层级（包括股权层级）的债券，这些债券以债券为抵押品创建了抵押债券义务（CBO）。以下哪一项是正确的？
\end{question}

\begin{choices}
    \item At fair value, the value of the issued bonds should be less than the collateral\\
    在公允价值下，发行的债券价值应小于抵押品
    ...
    在公允价值下，发行的债券按规模加权的总违约概率应等于抵押品池的违约概率
...
\begin{question}
    ...
    \\ 持续增加的违约概率（同时保持违约相关性不变）对夹层级和股权层的信用VaR最可能产生什么影响？
\end{question}
...
\begin{question}
    ...
    \end{itemize}
    哪一项关于证券化资产的信用增级，用于减轻信用风险和流动性风险的陈述是正确的？
    \begin{itemize}
        \item I. 内部和外部增级可以同时用于信用风险和流动性风险。
        \item II. 流动性风险储备类似于信用风险现金抵押账户。
    \end{itemize}
\end{question}
...
\begin{question}
    ...
    \\ 结构化证券化中的一个重要步骤是确定适当的信用支持或增级水平。以下哪一项不是证券化交易中常见的信用增级形式？
\end{question}
...
\begin{question}
    ...
    \\ 对冲基金正在考虑在抵押债务义务（CDO）的各种层级中采取头寸。基金的首席经济学家预测，违约概率将显著下降，而违约相关性将增加。基于这一预测，以下哪一项是值得追求的良好策略？
\end{question}
...
\begin{question}
    ...
    \\ 哪一项关于投资组合损失和违约相关性的陈述最可能是正确的？
    I. 增加违约相关性降低高级层价值，增加股权层价值。
    II. 在高违约率时，增加违约相关性降低夹层级债券价格。
\end{question}
...
\begin{question}
    ...
    \end{itemize}
    哪一项关于证券化资本结构陈述最可能是正确的？
    \begin{itemize}
        \item I. 夹层级通常是最小的层级。
        \item II. 夹层级和股权层通常提供固定票息。
        \item III. 高级层通常获得最低票息。
    \end{itemize}
\end{question}
...
\begin{question}
    ...
    \\ 一家公司正在经历财务困难。使用或有权益方法，哪一项最好描述了其高级债和次级债的估值？
\end{question}
...
\begin{choices}
    ...
    高级债对到期期限、公司波动率和利率都有负向敞口，次级债对到期期限、公司波动率和利率都有正向敞口
    ...
    高级债对到期期限、公司波动率和利率都有正向敞口，次级债对到期期限、公司波动率和利率都有负向敞口
...
\begin{question}
    ...
    \\ 在或有权益方法下，哪一项关于公司资本结构的陈述是正确的？假设高级债、次级债和股权的金额分别表示为F、U和S。
\end{question}
...
\begin{question}
    ...
    \\ 哈里·迈克尔，FRM，哈德森风险分析公司分析师，正在讨论股权、夹层和高级层级的违约敏感性，并做出以下陈述。哪一项陈述最可能是正确的？
    I. 违约敏感性在层级连接点附近最大。
    II. 违约敏感性是通过冲击信用违约互换（CDS）违约曲线计算的。
\end{question}
...
\begin{question}
    ...
    \\ 哈里·史密斯，XYZ银行公司首席财务官，正在考虑一项5亿美元的贷款证券化。他聘请了一位受人尊敬的结构化代理来帮助决定最有利的结构。XYZ是一家1000亿美元的地区银行，资产负债表适度强劲。其无担保债务的信用评级为BBB。最近，在隔离某些资产后，发行了信用评级为A的有担保债券。XYZ希望降低资金成本，并在新证券化的高级层级获得AAA信用评级。在审查XYZ的财务状况并预测未来经济条件后，结构化代理推荐了一种套利CDO，其损失分布如下：股权层级：0-30\%。次级层级：30-50\%。史密斯应该使用以下哪种CDO结构？
\end{question}
...
\begin{question}
    ...
    哪一项关于次级抵押贷款的陈述是正确的？
    \begin{itemize}
        \item I. 高级层由于超额抵押获得最高回报。
        \item II. 信用评级相同的公司债券和次贷资产池的损失波动性相同。
    \end{itemize}
\end{question}
...
\begin{question}
    ...
    \\经理做出了以下额外观察：
    \begin{itemize}
        \item 抵押贷款池中的贷款支付2.2\%的固定利差超过掉期曲线
        \item 第一年没有违约
        \item 第一年末超额抵押账户价值为EUR 0
    \end{itemize}
    如果第二年有8笔违约，超额抵押账户在第二年末的价值是多少？
\end{question}
...
\begin{question}
    ...
    假设MBS由以下四笔不同期限的抵押贷款组成：\$200万美元，期限90天；\$300万美元，期限180天；\$500万美元，期限270天；\$1000万美元，期限360天。这些抵押贷款池的加权平均期限（WAM）是多少？
\end{question}
...
\begin{question}
    ...
    假设抵押贷款支持证券的年提前还款率CPR为6\%，对应的单月死亡率SMM是多少？
\end{question}
...
\begin{question}
    ...
    证券化产品通常根据投资者和发起人的需求定制。哪一种资产支持证券（ABS）通常使用循环结构？
\end{question}
...
\begin{question}
    ...
    哪一项关于通过特殊目的实体（SPV）进行证券化结构化过程中信用增级的陈述是正确的？
\end{question}
...
\begin{choices}
    ...
    信用增级通常仅与抵押贷款支持证券（MBS）相关，而不用于其他类型的资产支持证券（ABS）
    ...
    最高层级票据通常被超额担保，以降低资产支持证券（ABS）的风险
    ...
    利差递增有时被资产支持证券（ABS）使用，在赎回日期后提高票息结构
\end{choices}
...
\begin{question}
    ...
    证券化对金融机构的主要好处之一是能够从资产负债表中移除资产，这降低了风险和所需的监管资本。虽然大部分风险从资产负债表中移除，但发起金融机构通常会保留部分风险。以下哪一项术语最好地识别发起人保留的风险？
\end{question}
...
\begin{question}
    ...
    在200\% PSA模型下，第10个月和第30个月的单月死亡率（SMM）分别是多少？
\end{question}
...
\begin{question}
    ...
    \end{itemize}
    作为零售银行的CRO，你正在向风险委员会展示证券化抵押贷款池的好处。以下哪一项将支持你认为这将有利于银行的论点？
    \begin{itemize}
        \item I. 它将提高你的银行的资本回报率
        \item II. 将立即增加你的银行的可用资本
        \item III. 你将能够向投资者提供有吸引力的收益率
        \item IV. 它将降低你的借款成本
    \end{itemize}
\end{question}
...
\begin{question}
    ...
    \\ 一个权重平均期限（WAM）为30个月的过手抵押贷款支持证券（MBS），本金余额为\$20亿美元。如果估值模型假设300\% PSA提前还款速度，哪一项最接近第一个月的提前还款（非计划）本金？
\end{question}
...
\begin{question}
    ...
    \\ 一个风险顾问正在审查一家美国初创信用卡公司的证券化提案。一位高级管理人员询问了证券化公司贷款组合的好处以及最适合信用卡债务组合的资产证券化结构类型。如果顾问建议证券化，哪一种证券化结构最合适，为什么？
\end{question}
...
\begin{choices}
    ...
    分期偿还结构，因为信用资产的本金支付以等额分期或到期一次性支付给投资者
    ...
    主信托结构，因为结构假设没有提前还款速度变化，投资者按票息支付本金和利息
\end{choices}
...
\begin{question}
    ...
    \\一家金融机构的固定收益投资组合经理在投资前正在估计MBS资产池的加权平均票息（WAC）。该资产池由四笔住宅抵押贷款组成。在指定月初的贷款信息如下：

    \begin{table}[H]
        \centering
        \begin{tabular}{|c|l|c|l|c|}
        \hline
        贷款 & 期初未偿余额 & 利率（票息） & 计划本金支付 & 剩余期限（月） \\
        \hline
        1 & USD 240,000 & 6.5\% & USD 18,800 & 16 \\
        \hline
        2 & USD 525,000 & 5.0\% & USD 23,000 & 34 \\
...
\begin{question}
    ...
    \\    一家对冲基金的风险分析师正在评估该基金持有的以下头寸：
    \begin{itemize}
        \item GTU Bank发行的或有可转换债券（CoCos）；转换的触发事件是GTU Bank公开交易股票的价格达到指定阈值
        \item 由破产隔离特殊目的实体发行的MBS的优先档；证券化中的抵押贷款由GTU Bank发起
        \item 作为固定利率支付方的利率互换合约，Bank NPV作为浮动利率支付方；Bank NPV提供高质量政府债券作为抵押品
    \end{itemize}
    分析师考虑了一个场景，其中利率上升，GTU Bank的股价下跌。当分析师审查这些市场条件变化对三个头寸的影响时，以下哪项是正确的？
\end{question}

\begin{choices}
    \item The CoCos held by the hedge fund are less likely to be converted to stocks of GTU Bank\\
    对冲基金持有的CoCos不太可能转换为GTU Bank的股票
