...
% %\pagestyle{empty}
  ...
  \end{question}
      ...
      \textbf{C选项错误。} 
      \begin{itemize}
          \item 错误的：这些因子每个都是显著的，调整后的 $R^2$ 对于实证金融来说相对较高。
      \end{itemize}
      \end{explanation}
...
\end{question}
  ...
  \begin{explanation}
      ...
      \begin{itemize}
          ...
          \item Ang（强调 ours）：“VIX变化和股票回报之间的相关性是-39\%，所以当波动率上升时，股票表现不好。波动率和回报之间的负关系被称为杠杆效应。当股票回报下降时，公司的财务杠杆增加，因为债务大致保持不变，而股权的市场价值下降。这使得股权风险更大，增加了它们的波动性。另一个通道是高波动率导致低股票回报：\textbf{波动率上升提高了投资者要求的股权回报，也导致股票价格下降。}这个第二个通道是一个时间变化的风险溢价故事，是基本CAPM倡导的：随着市场波动率增加，折现率增加，股票价格必须今天下降，以便未来股票回报可以高。” ($\dagger$)
          \item 关于（A）、（B）和（C），每项都是错误的。
      \end{itemize}
  ...
  \begin{explanation}
      \textbf{D选项错误。} 
      \begin{itemize}
          \item 错误的：Vasicek是一个单因素模型；为了对冲短期、中期和长期债券，需要例如三因素Gauss+模型。
      \end{itemize}
  ...
  \begin{explanation}
      \textbf{A选项错误。} 
      \begin{itemize}
          \item 正确的：公司C拥有最高的ROE，因为它有较低的股权地位。
      \begin{itemize}
  ...
  \begin{explanation}
      \textbf{A选项错误。} 
      \begin{itemize}
          \item 错误的：两项。相反，以下是正确的：风险偏好（即风险偏好声明）由高级管理层开发并由董事会批准。风险文化不是唯一责任，而是公司实际ORMF的基础，因此所有员工都对文化承担一定责任。
      \end{itemize}
...
关于这只三年期零息债券，下列哪项说法是正确的？
  ...
  \begin{explanation}
      \textbf{C选项正确。} 
      \begin{itemize}
          \item 正确的：$6.00\%$。 $\text{TEY} = \text{after-tax return on tax-exempt investment} / (1 - \text{marginal tax rate})$; in this case, $\text{TEY} = 4.20\%/(1 - 0.30) = 6.00\%$
      \end{itemize}
  \end{explanation}
  ...
  \begin{explanation}
      \textbf{A选项正确。} 
      \begin{itemize}
          \item 正确的：回归对冲最小化了对冲头寸的利润和损失（P\&L）方差。
          \item 如Tuckman所解释（加粗为我们所强调）：“\textbf{回归对的最佳论据}实际上并不是先前关于债券收益率完全依赖回归模型变化的假设。将头寸的盈亏（P\&L）写为……公式6.11……其中负号表示，名义本金为正且收益率上升时，P\&L会减少。可以证明，公式(6.8)中的回归对冲能够最小化方差。换句话说，\textbf{只要将P\&L方差作为适当的风险度量，回归对冲就能最小化被对冲头寸的风险。}”——引自Tuckman, Bruce; Serrat, Angel.《固定收益证券》（Wiley Finance）（第159页），Wiley出版社，Kindle版。
      \end{itemize}
      \textbf{B选项错误。} 
      \begin{itemize}
  ...
  \begin{explanation}
      \textbf{D选项正确。} 
      \begin{itemize}
          \item Zapex 有一个远期货币合同头寸，其公允价值收益与交易对手的违约概率正相关
      \end{itemize}
      \begin{itemize}
          \item 反向风险（信用风险）是敞口与信用质量之间的不利相关性，因此反向风险是敞口与违约概率（信用质量）之间的正（负）相关性。这是反向风险，因为更大的敞口与较低的支付可能性相关。代数上，反向风险（或正向风险）是敞口/EAD和PD之间的一些依赖性。我们通常写$EL\% = PD \times EAD$，但假设PD和EAD之间是独立的，因此考虑WWR的一种简单方法是当$EL > PD \times EAD$时，因为$EL \neq PD \times EAD$也是非独立性的条件。
      \end{itemize}
