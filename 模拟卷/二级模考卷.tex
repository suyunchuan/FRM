...
% %\pagestyle{empty}
  ...
  \begin{explanation}
      \textbf{C选项正确。} 
      \begin{itemize}
          \item 若分布为正态分布，则 $97.5\%$ ES 可由 $99.0\%$ VaR 近似。
          \item     在这种情况下，我们可以使用 $\$30.0 * (4.0\% + 15\% * 2.33) = \$30.0 * 38.950\% = \$11.69$。
          \item 精确的 $97.5\%$ ES 为 $\$11.7210$，因此该数值被作为正确选项。
      \end{itemize}
  \end{explanation}
  
  \checklist{正态分布下期望损失（ES）与 VaR 的近似}{3}{2}
  ...
  \begin{explanation}
      \textbf{D选项正确。} 
      \begin{itemize}
          \item Zapex 在远期货币合约中持有正相关于对手方违约概率的标记市值收益。
      
          \item 反向风险（信用风险）是敞口与信用质量之间的不利相关性，因此反向风险是敞口与违约概率（信用质量）之间的正（负）相关性。这是反向风险，因为更大的敞口与较低的支付可能性相关。代数上，反向风险（或正向风险）是敞口/EAD和PD之间的一些依赖性。我们通常写$EL\% = PD \times EAD$，但假设PD和EAD之间是独立的，因此考虑WWR的一种简单方法是当$EL > PD \times EAD$时，因为$EL \neq PD \times EAD$也是非独立性的条件。
          \item 关于（A）、（B）和（C），每项都是正向风险的例子。
      \end{itemize}
  \end{explanation}
  ...
  \textbf{A选项正确。} 
  \begin{itemize}
      \item 在住宅房地产敞口中，标准法（SA）下，如果贷款价值比（LTV）落入较低的区间，则风险权重较低。
  
      \item 见下文。巴塞尔委员会（High-level summary of Basel III reforms, December 2017）写道：“对于住宅房地产敞口，已经开发了更多风险敏感的方法，其中风险权重根据抵押贷款的贷款价值比（而不是现有的单一风险权重）以及更好地反映市场结构差异的方式而变化。”
  \end{itemize}
  ...
  \begin{itemize}
    \item 关于（B）、（C）和（D），每项都是错误的。相反，以下是正确的：
    \begin{itemize}
  ...
  \checklist{巴塞尔 III 信用风险修订：标准法（SA）与 IRB}{3}{2}
  ...
  \begin{explanation}
      \textbf{B选项正确。} 
      \begin{itemize}
          \item 0.0757。因为 $(96.00 - 89.00)/92.50 = 0.075676$
      
      \item 根据Hull (24.1.2. Measuring Market Liquidity)，两种衡量买卖价差的方法是：
      \begin{itemize}
  ...
  \end{question}
      ...
      \textbf{C选项为假。} 
      \begin{itemize}
          \item 这些因子每个都是显著的，调整后的 $R^2$ 对于实证金融来说相对较高。
      \end{itemize}
      \end{explanation}
      
      \checklist{Fama-French 三因子回归结果的解读}{3}{2}
  ...
  \begin{explanation}
      \textbf{A选项正确。} 
      \begin{itemize}
          \item 正确的：买入看涨期权，卖出看跌期权，并卖空股票
      \end{itemize}
  \end{explanation}
  
  \checklist{期权套利策略}{3}{2}
  ...
  \begin{explanation}
        \textbf{A选项正确。} 
        \begin{itemize}
            \item 总CVA是边际CVA的总和，因此必须为 $-1.450 + (-1.550) = -3.000$。
            \item 增量CVA必须等于总（净额）CVA，因此，如果IRS是第二个，其增量CVA等于 $-3.000 - (-2.100) = -0.900$。
        \end{itemize}
  \end{explanation}
  
  \checklist{净额结算集下 CVA：边际 CVA 与增量 CVA}{3}{2}
...
\end{question}
  ...
  \begin{explanation}
      ...
      \end{itemize}
      \begin{itemize}
          \item 记住这些三个额外缓冲的基本要求：CCB需要 $+2.5\%$ CET1; G-SIB需要 $\{+1.0\%, +1.5\%, +2.0\%, +2.5\%, \text{or } +3.5\%\}$ 取决于银行的评分 (见https://trtl.bz/fsb-g-sibs); 和 CCyB 需要在零和 $2.5\%$ 之间。
      \end{itemize}
  \end{explanation}
  
  \checklist{巴塞尔 III 资本缓冲：CCB、G-SIB 与 CCyB}{3}{2}
  ...
  \begin{explanation}
      \textbf{C选项正确。} 
      \begin{itemize}
          \item 单日 $99.0\%$ VaR 由 $-0 + 1.43\% * 2.326 * \$3.0 \text{ million} = \$99,800$ 给出。
          \item 流动性成本（LC）由 $(31.00 - 29.00)/30.00 * 0.5 * \$3.0 \text{ million} = \$100,000$ 给出。
          \item 因此，单日 $99.0\%$ VaR 为 $99,800 + 100,000 = 199,800$
      \end{itemize}

  \end{explanation}
  
  \checklist{流动性调整价值风险（LVaR）}{3}{2}
  ...
  \begin{explanation}
      \textbf{D选项正确。} 
      \begin{itemize}
          \item 因为最优主动风险由 $IR/(2*\lambda)$ 给出，帕特丽莎在从 $0.750/(2*0.10) = 3.750$ 到 $0.750/(2*0.050) = 7.500$ 时，最优主动风险翻倍。
      \end{itemize}
  \end{explanation}
  
  \checklist{最优主动风险与风险厌恶}{3}{2}
  ...
  \begin{explanation}
      \textbf{D选项正确。} 
      \begin{itemize}
          \item 正确的：他的信用风险在所有引用的交易日期增加；即，1月日期（4日和31日）和所有2月日期（8日、9日和13日）
      \begin{itemize}
      ...
      \item 信用风险可以通过多种方式产生。文本($\dagger$)列出了七种：
      \begin{enumerate}
  ...
  \checklist{信用风险的来源：应收、预付款、或有求偿与存款}{3}{2}
  ...
  \begin{question}
      ...
      \begin{enumerate}[label=\Roman*.]
        \item 除了信用风险和市场风险之外，巴塞尔II 还要求对操作风险计提资本
        \item 巴塞尔II 取消了风险权重和风险加权资产（RWA），改为直接计算风险资本占用
        \item 巴塞尔II 包含与资本和风险管理相关的监管要求（第二支柱），并要求公开披露（第三支柱）
        \item 为了对协议设计进行微调，巴塞尔II 多次采用量化影响研究（QIS），银行提供数据（反馈），由监管方进行分析
      \end{enumerate}
     Mary 的说法正确吗？
    \end{question}
  ...
  \begin{explanation}
      \textbf{C选项正确。} 
      \begin{itemize}
          \item 巴塞尔II并没有取消风险加权资产（或风险权重）；风险加权资产（RWA）在巴塞尔III/IV中仍然存在。根据GARP的Mark Carey，作为对原始巴塞尔I协议的改进，巴塞尔II的四项重大创新包括：更为复杂的风险权重公式；新增了操作风险资本要求；引入了第二支柱和第三支柱；以及多次采用量化影响研究（QIS）。许多专家认为新增的支柱是十分重大的进展。
          \item Carey解释道：“三大支柱代表了各国监管实践趋同的努力，特别是在‘规则与原则’分歧的背景下。具体来说，第二支柱要求监管者让银行持有高于最低要求的资本，并建立与其风险状况相符的内部资本充足评估流程（ICAAP）…… 协议条款执行方面的国家自主权被削弱，国家监管机构须对其执行协议的努力保持透明，包括那些高于最低要求的措施。第三支柱则要求进行更多定性和定量的信息披露，希望市场参与者的压力能帮助提升银行的实践水平（即市场约束）。”（来源：2020 FRM Part II: Operational Risk and Resiliency，第10版，Pearson Learning Solutions）
      \end{itemize}
  \end{explanation}
  ...
  \begin{question}
      ...
      假设美元/日元即期汇率是110.00，可以表示为USDJPY 110.00或JPY 110.00，其中USD是基础货币，JPY是报价货币；即，报价110.00日元每单位基础货币美元。美元利率为2.50\%，日元利率为0.450\%。期限为一年，复利频率为年复利。如果套利框架中的覆盖利率（CIP）强制执行一个完美准确的远期汇率，那么什么是隐含的一年“掉期率”，这里简单定义为远期汇率和即期汇率的差值；即，$F(\text{USDJPY}) - S(\text{USDJPY})$，或 $F(\text{USDJPY}) - 110.00$?
  \end{question}
  
  \begin{choices}
      \item -2.20 JPY (or -2.20 USDJPY)
      \\-2.20 日元（或 -2.20 USDJPY）
      \item Zero
      \\零
      \item + 5.80 JPY
      \\+ 5.80 日元
  ...
  \begin{explanation}
      \textbf{A选项正确。} 
      \begin{itemize}
          \item  $F - S = -2.20$ USDJPY
      
          \item 如果CIP被强制执行，那么 $F - S = S \times [(1 + \text{Rf\_JPY})/(1 + \text{Rf\_USD}) - 1] = 110.00 \times [(1 + 0.45\%)/(1 + 2.50\%) - 1] = -2.20$。
      \end{itemize}
  \end{explanation}
  
  \checklist{抛补利率平价（CIP）与远期汇率}{3}{2}
  ...
  \begin{explanation}
      \textbf{D选项正确。} 
      \begin{itemize}
          \item 股票的波动率增加（伴随着与市场的相关性增加），而市场波动率保持不变。
          \item 股票的波动率增加（伴随着与市场的相关性增加），而市场波动率保持不变（aka, ceteris paribus），意味着股票的beta增加。这增加了股票的预期回报。因此，股票的价格应该下降。
          \item Ang（强调 ours）：“VIX变化和股票回报之间的相关性是-39\%，所以当波动率上升时，股票表现不好。波动率和回报之间的负关系被称为杠杆效应。当股票回报下降时，公司的财务杠杆增加，因为债务大致保持不变，而股权的市场价值下降。这使得股权风险更大，增加了它们的波动性。另一个通道是高波动率导致低股票回报：波动率上升提高了投资者要求的股权回报，也导致股票价格下降。这个第二个通道是一个时间变化的风险溢价故事，是基本CAPM倡导的：随着市场波动率增加，折现率增加，股票价格必须今天下降，以便未来股票回报可以高。” ($\dagger$)
      \end{itemize}
      $(\dagger)$ \textbf{Andrew Ang, 《资产管理：因子投资系统方法》（纽约：牛津大学出版社，2014年）。}
  \end{explanation}
  
\checklist{波动率与股票回报：CAPM 风险溢价通道}{3}{2}
  ...
  \begin{explanation}
      \textbf{D选项错误。} 
      \begin{itemize}
          \item Vasicek是一个单因素模型；为了对冲短期、中期和长期债券，需要例如三因素Gauss+模型。
      \end{itemize}
      
  \end{explanation}
  
  \checklist{Vasicek 模型}{3}{2}
  ...
  \checklist{信用损失与杠杆对 ROE 的影响}{3}{2}
  ...
  \begin{explanation}
      \textbf{A选项错误。} 
      \begin{itemize}
          \item 风险偏好（即风险偏好声明）由高级管理层开发并由董事会批准。风险文化不是唯一责任，而是公司实际ORMF的基础，因此所有员工都对文化承担一定责任。
      \end{itemize}
      
  \end{explanation}
  
  \checklist{三道防线模型}{3}{2}
  ...
  \begin{explanation}
      \textbf{A选项正确。} 
      \begin{itemize}
      \item 错误的：流动性EWI是：债务/CDS利差扩大（与债务成本突然增加一致）
      \item 关于（B）、（C）和（D），每项都是流动性EWI的候选，根据研究笔记：
      \item ``巴塞尔银行监管委员会（BCBS）建议将以下措施作为流动性早期预警指标（EWI）。这些措施包括[重点摘要如下]：
      \begin{itemize}
  ...
  \begin{question}
      ...
      \end{itemize}
      Which is nearest to the asset's risk premium which refers to the asset's expected excess return, $E(r_i) - R_f$?
  ...
  \begin{explanation}
      \textbf{D选项正确。} 
      \begin{itemize}
          \item 预期超额回报 $E(r_i) - R_f = \beta(i,m) \times \lambda(m)$，其中 $\beta(i,m) = \text{cov}(r_i, m)/\text{var}(m)$，而 $\lambda(m) = -\text{var}(m)/E(m)$。
          \item 在本题中，$\beta(i,m) = \text{cov}(r_i, m)/\text{var}(m) = -0.10680/0.090 = -1.186667$，$\lambda(m) = -\text{var}(m)/E(m) = -0.090/0.9709 = -0.0926975$。
          \item 因此，$E(r_i) - R_f = \beta(i,m) \times \lambda(m) = -1.186667 \times -0.0926975 = 0.1100011$，约等于 $11.0\%$。
      \end{itemize}
  \end{explanation}
...
关于这只三年期零息债券，下列哪项说法是正确的？
  ...
  \begin{explanation}
      \textbf{B选项正确。} 
      \begin{itemize}
          \item 正确：接下来一年的预期回报是$9.00\%$。
          \item 我们不需要套用公式。每一年久期风险的风险溢价为50个基点，而这只三年期债券具有两年的久期风险。因此预期回报比$8.0\%$高出100个基点。
      \end{itemize}
  \end{explanation}
  
  \checklist{利率树下零息债券的久期风险溢价与预期回报}{3}{2}
  ...
  \begin{explanation}
      \textbf{C选项正确。} 
      \begin{itemize}
          \item 在这个场景中，所有批准文件都在会议前恰当地分发给了委员会。Jan确实花了时间向委员会解释投资，但她还帮助促进了委员会成员之间的讨论，并听取了每个人的意见。投票不必一致通过，但除非决定一致通过，否则应该举行投票。然而，会议应该准确记录，以防投资出现问题。在不良交易的情况下，审计师希望知道会议中讨论了什么，以及投资决策背后的推理。
      \end{itemize}
  \end{explanation}
  
  \checklist{信用委员会会议的潜在问题}{3}{2}
  ...
  \end{question}
      ...
      \textbf{C选项正确。} 
      \begin{itemize}
          \item 巴塞尔2.5市场风险资本要求需要99.0\%的置信水平，由以下公式给出：
      ...
      \checklist{巴塞尔 2.5 市场风险资本：VaR 与压力 VaR（SVaR）}{3}{2}
  ...
  \begin{explanation}
      \textbf{C选项正确。} 
      \begin{itemize}
          \item $\text{TEY} = \text{after-tax return on tax-exempt investment} / (1 - \text{marginal tax rate})$; in this case, $\text{TEY} = 4.20\%/(1 - 0.30) = 6.00\%$
      \end{itemize}
  \end{explanation}
  
  \checklist{税等效收益率（TEY）}{1}{1}
  ...
  \begin{question}
      ...
      安还声称，三个最重要的宏观因素是增长、通货膨胀和波动性。他对这些宏观因素的评估是基于一个长期的历史样本（具体来说，1952年第一季度至2011年第四季度）。重要的是要限定样本窗口，因为我们不能确定过去是序言；例如，利率在这段时间内经历了两个长期的大趋势。
  ...
  \begin{explanation}
        \textbf{C选项错误。} 
        \begin{itemize}
            \item 应该改为：在高通货膨胀期间，所有五个资产类别都比在低通货膨胀期间表现显著更差。
        \end{itemize}
      
  \end{explanation}
  
  \checklist{宏观因素（增长、通胀、波动）与资产表现}{3}{2}
  ...
  \begin{explanation}
      \textbf{C选项正确。} 
      \begin{itemize}
          \item 模型返回市场价格，如果我们使用贴现率为 2.25\%（时点 0，状态 0）、1.75\%（时点 1，状态 0）和 2.75\%（时点 1，状态 1）
      \end{itemize}
  \end{explanation}
  
  \checklist{期权调整利差（OAS）与 CMT 互换估值}{3}{2}
  ...
  \begin{question}
      ...
      在此情况下，Strongcore 不想计算标准的信用估值调整（CVA）公式，即对多个未来时点的贴现预期敞口 $EE(t)$ 求和。相反，Strongcore 希望使用更简化的“半解析方法”，该方法给出一个依赖于头寸当前价值、累积违约概率 $PD(0,T)$ 以及违约损失率 $LGD$ 的估计。在这种更简单的近似下，下列哪一个数值与该多头期权头寸的 CVA 费用最接近？
  \end{question}
  ...
  \begin{explanation}
      \textbf{C选项正确。} 
      \begin{itemize}
          \item $CVA = -LGD \times PD(0,T) \times V(t)$。本题中，$V(t) = S \times N(d1) - K \times \exp(-rt) \times N(d2) = 25 \times 0.66750 - 25 \times \exp(-0.030 \times 3) \times 0.53450 = \$4.475$ 百万美元。累积 $PD = 1 - \exp(-0.060 \times 3) = 16.473\%$。因此，CVA 的近似计算为 $70\% \times 16.473\% \times 4.475 \text{ 百万美元} = \$0.5160 \text{ 百万美元}$，约为 \$516,002。
      \end{itemize}
  \end{explanation}
  
  \checklist{CVA 近似：LGD × 累积违约概率 × 头寸价值}{3}{2}
  ...
  \begin{explanation}
      ...
      \textbf{D选项正确。} 
      \begin{itemize}
          \item 大多数运营风险的原因归因于控制弱点；人类偏见和行为失败；以及/或 operating environment changes
      \end{itemize}
  \end{explanation}

  \checklist{运营风险的成因与特征}{3}{2}
  ...
  \begin{explanation}
      ...
      \begin{itemize}
          ...
          \item 非存款服务销售收入 = $10$;
      \end{itemize}
      \item 总流入为 $\$100 + \$60 + \$80 + \$30 + \$10 = 280.0$ 百万。
      ...
      \item 总流出为 $\$70 + \$90 + \$20 + \$50 + \$40 = \$270.0$ 百万

      \item 因此，预计的净流动性头寸为 $\$280 - 270 = +10.0$ 百万。
      \end{itemize}
  \end{explanation}
  
  \checklist{流动性头寸的计算}{3}{2}
  ...
  \begin{explanation}
      \textbf{C选项正确。} 
      \begin{itemize}
          \item 混合 \#3 分配给科技ETF 65.0\%
          \item 当边际VaR保持不变时，风险被最小化（即，投资组合的分散VaR被最小化）。在四种混合中，混合 \#3 显然是边际VaR最接近的选择（即，$0.231$ 与 $0.232$ 几乎完全匹配）。
          \item 混合 \#1 具有最大的分散VaR。从混合 \#1 开始，风险通过添加边际VaR较低的组件（在本混合中，科技ETF的边际VaR仅为 $0.020$，而REIT ETF的边际VaR为 $0.461$）并减少边际VaR较高的组件来减少。
          \item 如果我们从混合 \#1 开始，并添加 $15.0\%$ 到科技ETF，那么我们“到达”混合 \#2。然而，在混合 \#2 中，REIT ETF的边际VaR仍然比科技ETF的边际VaR高得多（即，$0.386$ 与 $0.118$），因此我们可以继续向边际VaR较低的组件（仍然是科技ETF）转移。
          \item 如果我们再添加 $15\%$ 到科技ETF，那么我们到达混合 \#3：这里边际VaR几乎相等，因此我们非常接近于最小化投资组合的分散VaR的分配。记录在案，四种混合的分散投资组合VaR分别为 $\$6,131.14$（混合 \#1）、$\$5,043.03$（混合 \#2）、$\$4,621.42$（混合 \#3）和 $\$5,036.58$（混合 \#4）。
      \end{itemize}
  \end{explanation}
  
  \checklist{边际 VaR 与分散 VaR 最小化}{3}{2} 
  ...
  \begin{explanation}
      \textbf{A选项正确。} 
      \begin{itemize}
          \item 如Tuckman所解释（加粗为我们所强调）：“回归对的最佳论据实际上并不是先前关于债券收益率完全依赖回归模型变化的假设。将头寸的盈亏（P\&L）写为……公式6.11……其中负号表示，名义本金为正且收益率上升时，P\&L会减少。可以证明，公式(6.8)中的回归对冲能够最小化方差。换句话说，只要将P\&L方差作为适当的风险度量，回归对冲就能最小化被对冲头寸的风险。”——引自Tuckman, Bruce; Serrat, Angel.《固定收益证券》（Wiley Finance）（第159页），Wiley出版社，Kindle版。
      \end{itemize}
      \textbf{B选项错误。} 
      \begin{itemize}
  ...
  \begin{question}
      ...
      根据FDIC，模型风险管理始于强大的模型开发、实施和使用（aka，第一个元素。第二个元素是一个健全的验证过程。第三个元素是治理）。下列哪一项是关于模型风险管理 \textit{第一个元素}（开发、实施和使用）的正确陈述？
  \end{question}
  ...
  \begin{explanation}
    ...
    \textbf{D选项正确。} 
    \begin{itemize}
        \item 尽管测试的性质因上下文和模型类型而异，但测试是模型开发不可或缺的一部分
    \end{itemize}   
  \end{explanation}
  
  \checklist{模型风险管理}{3}{2}
  ...
  \begin{explanation}
      \textbf{B选项正确。} 
      \begin{itemize}
          \item 利息敏感缺口（Dollar IS GAP）= 利息敏感资产（ISA）- 利息敏感负债（ISL）= $\$490.0 - \$610.0 = -120.0$。
          \item 相对利息敏感缺口（Relative IS GAP）= $-120.0/490.0 = -0.245$。
          \item 利息敏感比率（ISR）= $490.0/610.0 = 0.803$。
          \item 这个银行是负债敏感的，因此上升的利率将降低其净利息收入（NIM）。
      \end{itemize}
  \end{explanation}
  
    \checklist{利息敏感缺口（IS GAP）}{3}{2}
  ...
  \begin{explanation}
      \textbf{A选项正确。} 
      \begin{itemize}
          \item 仔细应用包括特定问题列表的综合检查清单
      \end{itemize}
      
      \begin{itemize}
          \item 作者建议审查SEC网站，包括其问题列表；例如，投资欺诈红旗检查清单 at https://www.investor.gov/.
      \end{itemize}
      \textbf{B选项错误。} 
      \begin{itemize}
  ...
  \begin{explanation}
      \textbf{B选项正确。} 
      \begin{itemize}
          \item 将各种（甚至不寻常）单变量边际分布映射到具有可访问或熟悉依赖结构的分布，指定一个多元分布作为这些单变量边际分布的函数
      \end{itemize}
  \end{explanation}
  
  \checklist{Copula 函数的作用}{3}{2}
  ...
  \begin{explanation}
      \textbf{D选项正确。} 
      \begin{itemize}
          \item 这通常是企业信用风险的一个特征：投资组合风险由由于暴露集中而导致信用损失上升到意外水平的风险主导。
      \end{itemize}
      \begin{itemize}
  ...
  \checklist{零售信用风险与企业信用风险的差异}{3}{2}
  ...
  \begin{explanation}
      ...
      \textbf{D选项正确。}
      \begin{itemize}
          \item 这里所说的“渗透率”这一指标并不合理（用它来比较监管机构的有效性也同样如此），不过巴塞尔委员会多次讨论了渗透测试（penetration testing）作为网络风险有效的控制方法。不过需要注意，渗透测试（按类型：测试次数和发现等级）确实是一种常见且有效的网络弹性度量指标。详见4.2.2渗透测试部分。
      \end{itemize}
  \end{explanation}
  
  \checklist{网络安全和弹性指标}{3}{2}
  ...
  \begin{explanation}
      ...
      \textbf{D选项正确。} 
      \begin{itemize}
          \item 日内流动性风险应该纳入风险分类，是风险自评估的一部分，包括结算风险。
      \end{itemize}
  \end{explanation}
  
  \checklist{日内流动性风险管理框架}{3}{2}
...
\begin{question}
    ...
    Meissner 对道琼斯工业平均指数（DJIA）30只股票的股权相关性进行了详细研究，持续了44.5年（即，1972年1月至2017年7月534个月），产生了 $534 * 30^2 = 480,600$ 个相关值。在关于他对股权相关性的经验发现中，下列陈述中哪一项是正确，除了哪一项是错误？
\end{question}
...
\begin{explanation}
    \textbf{D选项错误。} 
    \begin{itemize}
        \item 相反，自相关倾向于随着更大的滞后而衰减（即，减少）。
    \end{itemize}
\end{explanation}

\checklist{股权相关性的经验特征与自相关衰减}{3}{2}
...
\begin{explanation}
    \textbf{D选项错误。} 
    \begin{itemize}
        \item 相反，CVA 是可分离的，这一点具有实际重要性。
    \item 最基本的、关键的关系是：$\text{风险价值} = \text{无风险价值} - \text{CVA}$。
\end{itemize}
\end{explanation}

\checklist{CVA 计算原则与可分离性}{3}{2}
...
\begin{explanation}
    \textbf{A选项正确。} 
    \begin{itemize}
        \item 损益（P\&L）中针对潜在损失影响计提的准备金或储备应计入总损失计算
        \item  摘自“总损失、净损失及回收的定义”（6. 损失数据识别、收集与处理的具体标准）：
        \begin{itemize}
       
        \item 21. 总损失是指在任何类型回收前的损失。净损失是指在考虑回收影响后的损失。回收是与原始损失事件相关、在时间上分开的独立事件，由第三方支付资金或经济利益流入的情况。
        
        \item 22. 银行必须能够识别所有操作损失事件的总损失金额、非保险回收和保险回收。银行应在损失数据集中使用扣除回收后的净损失（包括保险回收）。但只有在银行实际收到回收款后，回收才能用于减少损失。应收账款不计作回收。收到回收款以净额结算损失的验证材料须根据监管要求提供。
        
        \item 23. 损失数据集中总损失计算必须包括以下项目：
        \begin{itemize}
        \item (a) 由于操作风险事件对银行损益账户的直接计提，包括减值和和解，以及相关的资产减记；
        
        \item (b) 作为事件后果产生的费用，包括与操作风险事件直接相关的外部支出（如与事件直接相关的法律费用及支付给顾问、律师或供应商的费用）及为恢复事件发生前状态而产生的修理或替换费用；
        
        \item (c) 损益中就潜在操作损失影响计提的准备金或储备；
        
        \item (d) 因操作风险事件而产生具有明确财务影响的损失，这些损失暂时计入过渡和/或暂记账户，在损益尚未反映（“待决损失”）。重大待决损失应在符合其金额和时间要求的期限内计入数据集；
        
        \item (e) 某一财务会计期间因操作风险事件影响以前期间现金流或财务报表而记入的负面经济影响（“跨期损失”）。当操作风险事件跨越多个财务会计期间并引发法律风险时，重大“跨期损失”应计入损失数据集。”
        \end{itemize}
...
\checklist{操作风险损失数据：总损失与回收（SMA）}{3}{2}
...
\begin{explanation}
    \textbf{C选项错误。} 
    \begin{itemize}
        \item 风险计划应该对“重大、可预测且可避免”的错误/损失零容忍。
        \item 关于（A）、（B）和（D），每项都是正确的。根据作者，风险计划中的五个指导原则是：
    \begin{itemize}
...
\checklist{风险计划要素与可避免损失的零容忍}{3}{2}
...
\begin{explanation}
    \textbf{B选项正确。} 
    \begin{itemize}
        \item Bertha 想要“做空相关性”，并且随着相关性的下降，更好的两个选项变得更有价值。
        \item 另一方面：
    \begin{itemize}
...
\begin{explanation}
    \textbf{D选项正确。} 
    \begin{itemize}
        \item 她正在使用蒙特卡洛模拟，因为合同的未来价值高度不确定。
    \end{itemize}
    
    \begin{itemize}
        \item 根据 Jon Gregory 的说法，对手方信用风险与贷款风险之间的两个主要区别是：（1）贷款方知道其所承担的风险金额，但在对手方信用风险的情况下，风险金额高度不确定；（2）贷款通常只涉及单方风险（由贷款方承担），而对手方信用风险则是双边的，即双方都面临风险。
    \end{itemize}

    \begin{itemize}
        \item 关于错误选项（a），Herstatt 风险是结算风险的一种特殊情形（跨币种），而不是预结算风险。
    \end{itemize}
\end{explanation}

    \checklist{对手方信用风险特征}{3}{2}
...
\begin{explanation}
    \textbf{C选项正确。} 
    \begin{itemize}
        \item 跨境金融的增长要求银行在压力时期保持偿付能力，并要求在彼此本国市场竞争的银行之间实现公平竞争（即“公平竞争环境”）；因此其动机是银行安全性和竞争方面的考量。
   
        \item Carey 解释道：“有两个事件促成了巴塞尔I的诞生。
    \begin{itemize}
...
\begin{explanation}
    \textbf{C选项正确。} 
    \begin{itemize}
        \item 偶然性流动性捕捉现金或资产，当压力情景出现时可用。流动性取决于压力情景的发生。包括在这种情况下的金融资产，可以在不损失价值的情况下转换为现金。捕捉偶然性流动性以覆盖压力现金流出是流动性压力测试的主要原因。
    \end{itemize}

    \begin{itemize}
        \item Venkat 写道：“\textbf{流动性资产缓冲}：流动性资产缓冲代表当前存在的偶然性流动性... 流动性资产缓冲证券的基本特征应包括低信用和市场风险、估值的易性和确定性、在活跃且规模较大的市场上交易、以及买家和卖家集中度低。流动性资产缓冲还应满足运营要求，确保流动性在实体承受压力测试时处于中央 Treasury 部门的控制之下。”($\dagger$)
    \end{itemize}
    \begin{figure}[H]
        \centering
...
\begin{explanation}
   ...
   \textbf{D选项错误。} 
   \begin{itemize}
       \item 相反，每个投资组合经理的主观哲学陈述总是必要的，作为量化工具的补充，以澄清是否存在风格漂移。
   \end{itemize}

   \begin{itemize}
       \item 根据 GARP 的阅读，显式（和主观）的哲学陈述总是量化工具的必要补充，因为除了帮助识别 \textit{风格漂移} 之外，没有其他原因。摘自阅读：“在讨论这些[即 commonly used performance tools]之前，值得注意的是，尽管性能工具是必要的，但它们并不是在出现异常行为迹象时及时进行管理干预的替代品。当异常行为以 poor performance statistics 的形式显现时，损害可能已经无法挽回。因此，我们相信性能工具必须补充：
   \begin{itemize}
...
\begin{explanation}
    \textbf{B选项错误。} 
    \begin{itemize}
        \item 相反，映射是专门解决数据不足问题的解决方案。根据 Jorion 的说法，“风险经理必须用系统中已经存在的类似风险因素的敞口替换这些头寸。”$_{(\dagger)}$
    \end{itemize}
    （$\dagger$）Philippe Jorion，《价值-at-风险：金融风险管理的新基准》，第3版。（纽约：麦格劳-希尔，2007年）
\end{explanation}

\checklist{VaR 映射与风险因子}{3}{2}
...
\begin{explanation}
    \textbf{A选项正确。} 
    \begin{itemize}
        \item 当前敞口 = $\max(\text{MtM value}, 0)$。在这种情况下，$\text{CE} = \max(-335,900, 0) = 0$。Freecone 在互换中处于亏损状态；其对手方由于对手方在违约事件中遭受损失，因此具有正的当前敞口。但 Freecone 在违约事件中不会遭受当前损失。
    \end{itemize}
\end{explanation}

\checklist{对手方当前敞口}{3}{2}
...
\begin{explanation}
    \textbf{C选项正确。} 
    \begin{itemize}
        \item C选项并未提及对ESG因素的审查。
        \item 根据美联储理事会的指导，尽职调查应涵盖以下三大类：
        \begin{enumerate}
            \item 业务背景、声誉和战略： ``金融机构应当审查潜在服务提供商在行业内的地位及其公司历史与资质；审查服务提供商及其高级管理人员的背景与声誉；并确保服务提供商为其员工建立了合适的背景审查程序。''
            \item 财务状况与表现： ``金融机构应审查服务提供商及其密切相关关联公司的财务状况。财务审查可包括：服务提供商最近的财务报表和年度报告，涵盖未了承诺、资本实力、流动性及经营业绩；还应考虑服务提供商的可持续性，包括该服务提供商经营年限以及其在特定服务领域的市场份额增长等因素……''
            \item 运营与内部控制。
        \end{enumerate}
    \end{itemize}
...
\textbf{B选项正确。} 
\begin{itemize}
    \item 贷款-存款（LTD）比率（当前和预测）与董事会批准的最高天花板（限制）$80.0\%$
\end{itemize}

\end{explanation}

\checklist{存款追踪与LTD上限}{3}{2}
...
\begin{explanation}
    ...
    \begin{itemize}
        \item 标签/描述存在三重不一致：
        \item （1）股票市场中性策略与可观察的市场风险因子无关（在引用的研究中，这是唯一没有与市场风险因子关联的策略）；因此，股票市场中性基金不可能“带来一篮子贝塔”，其目的正是中和贝塔，即对主流市场因子的贝塔为零；
        \item （2）此外，股票市场中性策略并不属于事件驱动型策略，它本身是非方向性的，被作者归类为“小众策略”（niche strategy），有时也归于股票对冲（equity hedge）策略。
        \item （3）描述将其归为事件驱动，也是错误的。
    \end{itemize}

    \begin{itemize}
        \item 关于（A）、（B）和（C），其他三只基金（Acheilus、Boreas 和 Chimera）的策略、宣传语及描述彼此一致，具有可信度。
    \end{itemize}
\end{explanation}

\checklist{对冲基金策略辨析}{3}{2}
...
\begin{explanation}
    \textbf{A选项正确。} 
    \begin{itemize}
        \item 为了增加回测的威力，她可以降低 VaR 置信水平并应用更高的（k）资本乘数。
        \item 一个关键的统计挑战是，高置信水平意味着异常样本量很小。解决方法是降低置信水平，但应用乘数（降低置信水平的 VaR）以近似高置信水平的 VaR。
        \item 根据 Jorion 的说法，“这个框架缺乏威力的原因是选择了高 VAR 置信水平（$99$ 百分比），这导致异常样本量太少，无法进行可靠的测试。相比之下，考虑 $95$ 百分比 VAR 置信水平的效果。（为了确保资本金额不受影响，我们可以使用更大的乘数 k。）” — Philippe Jorion. Value at Risk, 3rd Ed.: The New Benchmark for Managing Financial Risk (p. 150). Kindle Edition.
    \end{itemize}
    \textbf{B选项错误。} 
    \begin{itemize}
    ...
    \textbf{D选项错误。} 
    \begin{itemize}
        \item 尽管 VaR 总是单尾分位数（例如，$95.0\%$ 标准正态 VaR 总是 $1.645$ 且从不 $1.960$），但回测总是双尾（2T）回测，因此 Betty 必须有一个下限和一个上限
    \end{itemize}
\end{explanation}

\checklist{VaR 回测与统计检验}{3}{2}
...
\begin{explanation}
    \textbf{A选项正确。} 
    \begin{itemize}
        \item ß融资成本（FVA）为正。为了对冲，需要抵押品，这笔金额需要融资。
    \end{itemize}
    \textbf{B选项错误。} 
    \begin{itemize}
    ...
    \textbf{C选项错误。} 
    \begin{itemize}
        \item 他们的对冲不会消除对监管资本的需求！Gregory 说：“资本：会有资本要求，这些要求会变化——这很重要，因为资本是一种成本。无抵押交易的资本会随着成为实值而增加。对冲的资本不会抵消这一点，因为它是抵押的——即使它是无抵押的，也可能会有凸性影响（一笔交易的资本会增加超过另一笔交易的资本减少）。”
    \end{itemize}
    \textbf{D选项错误。} 
    \begin{itemize}
        \item 这里的问题是交易员需要购买（购买）而不是出售（卖出）一个 CCDS。Gregory 说：“更定制化的信用衍生产品，如或有 CDS（CCDS），已被设计为更直接地对冲对手方风险。CCDS 本质上是一种 CDS，但保护的名义金额与合同指定的衍生品（或甚至衍生品组合）的敞口挂钩。它们允许将特定交易和对手方的对手方风险合成转移给第三方。然而，CCDS 市场从未发展出任何重要的流动性。”
    \end{itemize}
    (来源：Jon Gregory，《xVA 挑战：对手方信用风险、融资、抵押和资本》，第3版（西萨塞克斯，英国：约翰·威利父子有限公司，2015年）)
\end{explanation}

\checklist{CVA对冲与融资成本}{3}{2}
...
\begin{explanation}
    \textbf{C选项错误。}
    \begin{itemize}
        \item turnover bias（换手率偏差）是杜撰出来的。
        \item 根据 Ang 的说法，对于流动性较差资产，使投资者高估预期回报并低估风险的三种偏差分别是：生存偏差、不频繁采样和选择偏差。
        \item 因此，turnover bias 降低了交易之间的典型时间，并倾向于人为地增加预期回报 5.0\% 或更多。
    \end{itemize}

\end{explanation}

\checklist{非流动性资产回报偏差}{3}{2}
...
\begin{question}
    Which of the following provides the BEST explicit link(s) between a bank’s contingency funding plan (CFP) and its liquidity stress testing framework?\\
        哪一项提供了银行应急融资计划（CFP）与其流动性压力测试框架之间最佳的显式链接？
    \end{question}
    ...
    \begin{explanation}
        \textbf{B选项正确。} 
        \begin{itemize}
            \item 解释 Lai 和 Tuosto 的观点（加重号为原文所有）：“II. 与更广泛的风险管理框架整合：CFP（应急融资计划）不是一个孤立的工具，而是机构流动性风险管理和集团范围风险管理框架的有机组成部分……CFP 应当与流动性风险度量框架和流动性压力测试框架明确地结合起来，尤其是通过其限额结构和升级级别。
            \item 举例来说，机构在日常（BAU）风险管理活动中使用的流动性风险度量，为 CFP 确定其预警指标（EWI）提供了基础。此外，与业务连续性和危机管理框架的关联，会在危机时期加强关键的运营和沟通流程。”——摘自 Chi Lai 和 Richard Tuosto，《Venkat 的流动性风险管理》中的《应急融资计划》
            \item 因此，限制结构和升级级别是银行 contingency funding plan（CFP）与其流动性压力测试框架之间最佳的显式链接。
        \end{itemize}
    \end{explanation}

    \checklist{应急融资计划与流动性压力测试}{3}{2}
...
\begin{explanation}
    \textbf{B选项正确。} 
    \begin{itemize}
        \item 在这种情况下，$0.5 \sigma(e) \sqrt{T} = 0.5 \times 36.0\% \times \sqrt{1} = 18.0\% = 0.180$ 和 $N(0.18) = 0.57140$ 如果按照查找表四舍五入。
        \item 然后 $c = 2 \times 0.57140 - 1 = 14.28\%$。
    \end{itemize}
\end{explanation}

\checklist{市场时机与BSM公式}{3}{2}
...
\begin{explanation}
    \textbf{B选项正确。}
    \begin{itemize}
        \item 首先需要求解 $k$。Dowd($\dagger$) 使用指数 $(-1/\xi)$，但我们将在指数中使用更简单的倒数 $(-\xi)$。这样，幂律说：$\text{Prob}(v > x) = F(x) \approx k x^{-\xi}$。求解常数 $k$，必须有：$k = F(x) x^{\xi}$。
        \item 在这种情况下，当 $x = 6.0$ 时 $F(x) = 0.50\%$ 且 $\xi = 4.0$，我们可以找到 $k = 0.50\% \cdot 6.0^4 = 6.480$。现在我们知道，因为我们假设常数 ($k$) 近似，我们的分布是 $F(x) \approx k x^{-\xi} = 6.480 x^{-4.0}$。
        \item 我们想要 $F(x) = 0.10\%$ 的 VaR，即 $0.10\% = 6.480 x^{-4.0}$，其中 $x = (6.480/0.10\%)^{1/4.0} = 8.97$，四舍五入到 \$9.0 百万。
    \end{itemize}
\end{explanation}

\checklist{极值理论与幂律VaR}{3}{2}
...
\begin{explanation}
    \textbf{C选项正确。}
    \begin{itemize}
        \item 没有净额结算时，总敞口 = $\max(0, +12) + \max(0, -9) + \max(0, 20) + \max(0, -6) + \max(0, +8) = 40$。
        \item 有净额结算时，总敞口 = $+12 - 9 + 20 - 6 + 8 = 25$。注意这与 $-(-9 - 6)$ 相同。
    \end{itemize}
\end{explanation}

\checklist{净额结算与敞口减少}{3}{2}
...
\begin{explanation}
    ...
    \textbf{B选项正确。}
    \begin{itemize}
        \item 她应当将自己公司生成的场景与外部来源发布的行业特定场景列表进行比较，但这种比较应在公司自行内部生成场景之后进行。根据GARP（第3.2节），“最后，公司比较自己所生成的场景与行业场景列表是有益的，可以判断是否遗漏了相关场景或风险驱动因素。比如，保险业操作风险协会（ORIC）和ORX等行业组织会向其成员提供现成的场景列表。然而，建议企业只在场景生成结束后，再将其自有场景与外部行业列表比较，而非事前进行，以避免外部场景影响公司的生成过程。” (\dag)
    \end{itemize}
...
\begin{explanation}
    \textbf{C选项正确。}
    \begin{itemize}
        \item 许多资产类别内部确实存在较大的流动性风险溢价，但不是跨资产类别。
    \end{itemize}
\end{explanation}

\checklist{流动性风险溢价}{3}{2}
...
\begin{explanation}
    ...
    \textbf{C选项正确。} 
    \begin{itemize}
        \item 她应该遵循所有步骤，但要有灵活性，因为尽职调查也是一种艺术，每个基金都有自己独特的策略。
        \item GARP 解释说：“对对冲基金进行尽职调查既像艺术又像科学。投资者必须使用全面的检查清单，确保没有任何遗漏或遗漏，同时保持自由提出开放式问题，以提供对公司哲学或文化的洞察。这里讨论的投资、运营和商业模型风险评估指南主要是通用的。在执行尽职调查时，投资者必须修改他们的方法，使其具有策略针对性。” (Summary, 2020 Financial Risk Management Part II: Risk Management and Investment Management, 10th Edition. Pearson Learning Solutions, 2019. VitalBook file.)
    \end{itemize}
...
\begin{explanation}
    \textbf{D选项正确。}
    \begin{itemize}
        \item 一日 $95.0\%$ 正态 VaR 大约比一日 $95.0\%$ 对数正态 VaR 多 \$448,800。
    \end{itemize}
    
    \begin{itemize}
        \item The one-day $95\%$ normal VaR(\%) is given by:
        $$-14\%/250 + 32\% \times 1/\sqrt{250} \times 1.645 = 3.273246\%$$
        \item 一日 $95.0\%$ 正态 VaR（\%）为：
        $$-14\%/250 + 32\% \times 1/\sqrt{250} \times 1.645 = 3.273246\%$$
        \item 一日 $95.0\%$ 对数正态 VaR（\%）为：
        $$1 - \exp(12\%/250 - 28\% \times 1/\sqrt{250} \times 1.645) = 2.824436\%$$
        \item 差异为 $3.273246\% - 2.824436\% = 0.448810\%$，或大约 $0.448810\% \times \$100.0 \text{ million} = \$448,810$；使用精确分位数，差异为 $\$448,765.91$。
    \end{itemize}
\end{explanation}

\checklist{正态与对数正态VaR}{3}{2}
...
\begin{explanation}
    \textbf{D选项正确。}
    \begin{itemize}
        \item 正确：B -> 25 -> C -> 10 -> A (新总名义金额 = 70)
    \end{itemize}
    
    \begin{itemize}
        \item 最简单的方法是识别最低敞口，在这种情况下为 50，并从所有三个中减去它。这样：
    \begin{itemize}
        \item 当前 A -> 50 -> B 被消除；
...
\begin{choices}
    ...
    he Swiss cheese\\
    模型很优雅，因为聚类及其中心（即“洞”）是通过 k-means 聚类定量获得的；但是，当需要把人因纳入整体风险视角时，它会遇到困难
\end{choices}
...
\begin{explanation}
    \textbf{B选项正确。}
    \begin{itemize}
        \item 工具是前瞻性的，因为它通过检视控制和间接原因，在风险尚未发生前评估风险的可能性和/或影响；然而，它本身缺少定量框架，因此优先级排序更困难
    \end{itemize}
\end{explanation}

\checklist{操作风险有因分析}{3}{3}
...
\begin{explanation}
    ...
    \textbf{D选项正确。}
    \begin{itemize}
        \item 联邦住房贷款银行（aka，FHLBanks）的贷款在储蓄机构和中小银行中非常受欢迎，作为稳定来源的灵活（相对于到期日）资金，以低于市场的利率
 
        \item 根据研究笔记：“联邦住房贷款银行（FHLB）系统近年来已进入向住房抵押贷款提供者贷款。由 12 个地区银行组成，FHLB 于 1932 年创建，作为存款机构面对存款人潜在挤兑时的一个稳定资金来源。根据许多学者，FHLB 在“资金来源”领域的作用不仅增加了使用 FHLB 作为资金来源的金融机构（以住房抵押品为担保）的可用资金量，而且还增加了住房贷款的流动性，并提供了激励更多金融机构向住房市场提供信贷。

        \item 近年来，FHLB 的影响力迅速扩大，现在已有 6,000 多家商业银行、1,300 多家储蓄机构、700 多家信用合作社和 80 多家保险公司有资格向 FHLB 借款。FHLB 资金的吸引力在于它是一个稳定的融资来源，并能提供低于市场利率的资金。金融机构可以从 FHLB 按灵活的时间期限借款，期限从隔夜到 20 多年不等，利率可以是固定或浮动的。FHLB 能够提供低于市场利率资金的部分原因是其联邦特许章程允许其低成本融资，并将这些节省下来的成本转嫁给会员银行，后者作为股东可获得分红。此外，在机构发生倒闭时，联邦住房贷款银行系统处于有利地位。根据法律规定，FHLB 在追偿资金方面优先于包括 FDIC 在内的其他机构。
    \end{itemize}
\end{explanation}

\checklist{非存款资金来源}{3}{2}
...
\begin{explanation}
    \textbf{D选项正确。}
    \begin{itemize}
        \item 动量策略的累积利润比规模或价值策略的累积利润大一个数量级。
    \end{itemize}
\end{explanation}

\checklist{动量与规模价值比较}{3}{2}
...
\begin{explanation}
    ...
    \textbf{D选项正确。}
    \begin{itemize}
        \item 非参数密度估计（NPDE）可以在细粒度置信水平（例如 $99.10\%$）下检索 VaR，就像分布是一个连续概率密度函数（pdf）
    \end{itemize}
\end{explanation}

\checklist{历史模拟方法辨析}{3}{2}
...
\begin{question}
    ...
    双方交易对手（A 和 B，见下图）之间存在一组场外（OTC）衍生品交易。
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.6\textwidth]{2·72.png} 
    \end{figure}
    针对这些头寸，双方在发起基础交易之前还签订了双边抵押协议。以下陈述均为真，除了哪一项为假？
\end{question}
...
\begin{explanation}
    \textbf{C选项为假。}
    \begin{itemize}
        \item 这是一项知识基础（KB）错误。根据阅读（GARP），知识基础错误的相应响应是指令控制。
    \end{itemize}
\end{explanation}

\checklist{操作风险人为错误分类}{3}{2}
...
\begin{explanation}
    \textbf{A选项错误。} 
    \begin{itemize}
        \item 因为银行参与期限转换（maturity transformation 是该章节反复出现的因果主题）。
    \end{itemize}
    \begin{itemize}
        \item 如 Grant 在3.1中所解释，为什么银行需要LTP：在日常业务中，银行通过用短期存款（负债）为长期贷款（资产）提供资金来赚钱，这一过程通常称为期限转换。正如巴塞尔银行监管委员会（BCBS，2008年9月）所指出的，这“使得银行本质上容易受到流动性风险的影响，无论是机构特定的流动性风险还是影响整个市场的流动性风险”（第3页）。
        \item 但只要银行在产品定价、新产品审批流程以及利润和绩效评估中使用LTP来计量流动性的成本、收益和风险，就不应阻止银行参与期限转换。
        \item 拥有糟糕LTP实践的银行更可能积累大量长期非流动性资产、有条件承诺以及短期波动性更大的负债，从而显著增加其资金短缺的脆弱性。（Joel Grant, 2011, “Liquidity Transfer Pricing: A Guide to Better Practice,” Occasional Paper, Financial Stability Board, Bank for International Settlements.）
    \end{itemize}
   
\end{explanation}

\checklist{流动性转移定价（LTP）}{3}{2}
...
\begin{explanation}
    \textbf{D选项为假。} 
    \begin{itemize}
        \item 相反，作者发现投资者并没有因为欺诈风险而得到补偿，无论是通过卓越表现和/或更高的费用
    \end{itemize}
\end{explanation}

\checklist{投资欺诈可预测性}{3}{2}
...
\begin{explanation}
    \textbf{A选项正确。} 
    \begin{itemize}
        \item 我们在任何 EVT 方法中都面临偏差-方差权衡。在块方法中，权衡是块大小（或数量）的反映。在 POT 方法中，它是任意阈值选择，类似于块大小选择。如果阈值太低，那么我们的模型将不符合 EVT 的渐近性质，模型可能不准确（aka，偏差）。更高的阈值意味着更低的偏差（aka，更高的准确性），但这必然适合更少的数据点，因此我们可以预期更高的方差（aka，更低的精度）。
   
        \item Dowd$_{(\dagger)}$ 在半参数估计方法（例如，Hill 估计器）的上下文中参考了偏差-方差权衡：“... 基于 $k$ 的选择意味着偏差和方差之间的权衡。如果我们增加 $k$，我们获得更多数据，因此移动到分布的中心。这增加了我们的估计器的精度（因此降低了它的方差），但也增加了尾部估计器的偏差，通过在分布中心附近放置相对更多的权重来观察。或者，如果我们减少 $k$ 并进一步沿着尾部移动，我们减少偏差，但有更少的数据来使用，并且得到更高的方差。”
    \end{itemize}
\end{explanation}

\checklist{极值理论POT阈值}{3}{2}
...
\begin{explanation}
    \textbf{A选项为假。} 
    \begin{itemize}
        \item 再抵押最小化融资成本。权衡是再抵押最小化融资成本但增加交易对手风险。
    \end{itemize}
\end{explanation}

\checklist{抵押品再抵押与融资成本}{3}{2}
...
\begin{explanation}
    ...
    \begin{itemize}
        \item 相反，百分比比美元更受欢迎。
        \item GARP 说：“报告运营损失作为毛收入、总成本（用于成本中心）、总预算（用于项目）或资本基点，有助于跨业务单位比较类似实体。报告以百分比而不是绝对值也使报告可扩展并适用于所有规模的实体。理想情况下，运营风险管理人员也将更好地理解实施适当控制和运营限制的成本效益评估和权衡。”({\dag})
    \end{itemize}
...
\textbf{C选项正确。} 
    \begin{itemize}
        \item 银行美元资金的有效期限缩短，而美元资产期限延长，这增加了它们的期限错配。
        \item McGuire (emphasis ours): “危机期间的事件导致了银行短期资金来源的严重中断。银行间市场陷入瘫痪，外汇掉期市场的混乱使得通过掉期获得美元的成本更高。银行的资金压力也因非银行资金来源的不稳定性而加剧，特别是美元货币市场基金和持有美元的中央银行。
        \item 市场压力意味着银行美元资金的有效期限缩短，而美元资产期限延长，因为许多资产在流动性不足的市场上难以出售。
        \item 这种内生性的期限错配，难以事前对冲，导致了全球美元短缺。我们对银行美元资金缺口在危机开始时的估计揭示了为什么国际政策响应是必要的，以及为什么它采取了全球中央银行掉期线的形式。” (Patrick McGuire, Gotz von Peter, 2009. “The US Dollar Shortage in Global Banking and the International Policy Response,” BIS Working Papers, Bank for International Settlements.)
    \end{itemize}
\end{explanation}

\checklist{全球银行美元短缺原因}{3}{2}
...
\begin{explanation}
    ...
    \textbf{B选项正确。} 
    \begin{itemize}
        \item 在现实中，直接投资于声誉是困难的，几乎不可能想象如何将策略操作化为可投资的因子。
        \item 根据 Andrew Ang（见 https://www.blackrock.com/us/individual/investment-ideas/what-is-factor-investing），有两种主要类型的因子：宏观经济因子和风格因子。宏观经济因子包括经济增长、信用（违约风险）、实际利率、新兴市场（政治和主权风险）、通胀和流动性。风格因子包括价值、质量、最小波动、规模、动量和carry。
    \end{itemize}
    ...
    \textbf{D选项恰当，不选。} 
    \begin{itemize}
        \item 实际利率和流动性是真正的宏观因子
    \end{itemize}
\end{explanation}

\checklist{因子投资组合优化}{3}{2}
